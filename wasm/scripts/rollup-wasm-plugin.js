...
async function wasm_pack(cx, state, dir, source, id, options) {
  ...
  const WASM_TARGET_DIR = $path.resolve(
    $path.join(target_dir, "wasm-pack", name)
  );
  ...
  try {
    ...
    if (e.code === "ENOENT") {
      throw new Error(
        "Could not find wasm-pack, install it with `yarn add --dev wasm-pack` or `npm install --save-dev wasm-pack`"
      );
  ...
  const import_path = JSON.stringify(
    "./" +
      posixPath(
        $path.relative(dir, $path.join(WASM_TARGET_DIR, `${PACKAGE_NAME}.js`))
      )
  );
  ...
  const wasmPath = posixPath(
    $path.relative(dir, $path.join(WASM_TARGET_DIR, "wasm.base64.js"))
  );
