...
function display_results_summary {
  ...
  stake_account_balance_total_sand="$(bc <<< "scale=3; $stake_account_balance_total/$LAMPORTS_PER_SAND")"
  system_account_balance_sand="$(bc <<< "scale=3; $system_account_balance/$LAMPORTS_PER_SAND")"

  all_account_total_balance="$(bc <<< "scale=3; $system_account_balance+$stake_account_balance_total")"
  all_account_total_balance_sand="$(bc <<< "scale=3; ($system_account_balance+$stake_account_balance_total)/$LAMPORTS_PER_SAND")"
  ...
  printf "Balance of all STAKE accounts: %'.3f SAND\n" "$stake_account_balance_total_sand"
  printf "\n"
  printf "Balance of SYSTEM account: %'d lamports\n" "$system_account_balance"
  printf "Balance of SYSTEM account: %'.3f SAND\n" "$system_account_balance_sand"
  printf "\n"
  printf "Total Balance of ALL accounts: %'d lamports\n" "$all_account_total_balance"
  printf "Total Balance of ALL accounts: %'.3f SAND\n" "$all_account_total_balance_sand"
  echo "--------------------------------------------------------------------------------------"
}
