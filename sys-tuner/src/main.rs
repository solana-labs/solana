...
fn tune_poh_service_priority(uid: u32) {
    ...
    if let Some(pid) = find_pid("safecoin-validato", "/proc", uid, |dir| {
        let mut path = dir.path();
        path.push("task");
...
fn main() {
    ...
    if let Err(e) = std::fs::remove_file(safecoin_sys_tuner::SAFECOIN_SYS_TUNER_PATH) {
        if e.kind() != std::io::ErrorKind::NotFound {
            panic!("Failed to remove stale socket file: {:?}", e)
        }
    }

    let listener = unix_socket::UnixListener::bind(safecoin_sys_tuner::SAFECOIN_SYS_TUNER_PATH)
    ...
    if let Some(user) = users::get_user_by_name(&user) {
        ...
        nix::unistd::chown(
            safecoin_sys_tuner::SAFECOIN_SYS_TUNER_PATH,
