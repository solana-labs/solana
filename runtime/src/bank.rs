...
use {
    ...
    std::{
        ...
        fmt, mem,
        ops::{AddAssign, Deref, Range, RangeInclusive},
        path::PathBuf,
        rc::Rc,
        sync::{
            atomic::{
...
pub struct SlotBlockHeight {
    /// the slot at which rewards distribution began
    /// only used for metrics
    start_slot: Slot,
    /// the block height at which rewards distribution began
        ...
        let (_, update_rewards_with_thread_pool_time) = measure!(
            {
                if self.partitioned_rewards_feature_enabled() {
                    ...
                    datapoint_warn!(
                        ...
                        ("slot", self.slot(), i64),
                        ("activate", 1, i64),
                        ("start_slot", parent_slot, i64),
                        ("start_height", parent_height, i64),
                    );

                    self.calculate_rewards_and_distribute_vote_rewards_with_thread_pool(
                        parent_epoch,
                        reward_calc_tracer,
                        &thread_pool,
                        &mut rewards_metrics,
