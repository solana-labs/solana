...
impl HashAgeKind {
    ...
    pub fn fee_calculator(&self) -> Option<Option<FeeCalculator>> {
        match self {
            Self::Extant => None,
            Self::DurableNoncePartial(_, account) => Some(nonce::utils::fee_calculator_of(account)),
            Self::DurableNonceFull(_, account, _) => Some(nonce::utils::fee_calculator_of(account)),
        }
    }
}
