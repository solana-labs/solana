...
impl Bank {
    ...
    pub(crate) fn prepare_rewrites_for_hash(&self) {
    ...
    fn collect_rent_from_accounts(
        ...
        for (pubkey, account, _loaded_slot) in accounts.iter_mut() {
            ...
            if !just_rewrites
                && ((!can_skip_rewrites && !test_skip_rewrites_but_include_hash_in_bank_hash)
                    || !Self::skip_rewrite(rent_collected_info.rent_amount, account))
            {
            ...
            } else if (test_skip_rewrites_but_include_hash_in_bank_hash || just_rewrites) && !can_skip_rewrites {
                ...
                // feature is enabled.
                // This code path is also required to load snapshots generated by validators that were skipping rewrites, when rewrites are still included in the bank hash.
