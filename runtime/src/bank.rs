                    ...
                    match r {
                        solana_scheduler::ExaminablePayload(solana_scheduler::Flushable::Payload((mut ee, timings))) => {
                            ...
                            if send_metrics && ee.finish_time.is_some() {
                                ...
                                info!("execute_substage: slot: {} transaction_index: {} timings: {:?}", latest_runner_context.as_ref().unwrap().slot(), ee.task.transaction_index(), timings);
                            }
                        ...
                        solana_scheduler::ExaminablePayload(solana_scheduler::Flushable::Flush(checkpoint)) => {
                            info!("post_execution_handler: slot: {:?} {:?}", latest_runner_context.as_ref().unwrap().slot(), transaction_error_counts.aggregate().into_iter().chain([("succeeded", succeeded), ("skipped", skipped)].into_iter()).filter(|&(k, v)| v > 0).collect::<std::collections::BTreeMap<_, _>>());
            ...
            .spawn({
                ...
                loop {
                    ...
                    let maybe_checkpoint = solana_scheduler::ScheduleStage::run(
                        ...
                        |runner_context| format!("id_{:016x}{}", random_id, runner_context.as_ref().map(|c| format!(" slot: {}", c.slot())).unwrap_or("".into())),
                    );
...
impl Bank {
    ...
    pub fn register_tick(self: &Arc<Self>, hash: &Hash) {
        assert!(
            !self.freeze_started(),
    ...
    */
    ...
    fn _hash_internal_state(&self, bank_hash_override: &mut Option<Hash>) -> Hash {
        ...
        info!(
            "bank frozen: {slot} (parent: {}) hash: {} accounts_delta: {} sigs: {} txs: {}, last_blockhash: {} capitalization: {}{}, stats: {bank_hash_stats:?}",
            self.parent_slot(),
