...
use {
    crate::{
        ...
        cached_executors::{CachedExecutors, MAX_CACHED_EXECUTORS},
...
impl Bank {
    ...
    pub(crate) fn get_executors(&self, accounts: &[TransactionAccount]) -> Rc<RefCell<Executors>> {
    ...
    pub(crate) fn store_missing_executors(&self, executors: &RefCell<Executors>) {
        self.store_executors_internal(executors, |e| e.is_missing())
    }

    /// Add updated executors back to the bank's cache
    pub(crate) fn store_updated_executors(&self, executors: &RefCell<Executors>) {
    ...
    pub(crate) fn remove_executor(&self, pubkey: &Pubkey) {
