...
use solana_accounts_db::accounts_hash::AccountLTHash;
#[allow(deprecated)]
...
pub struct Bank {
    ...
    /// the pubkey, (account, hash) pairs that were loaded in this slot in order
    /// to be written. Ideally, when the bank is done, this contains a
    /// calculated hash value for all accounts which are written in THIS slot.
    pub old_written_accounts:
        RwLock<HashMap<Pubkey, (Option<AccountSharedData>, Option<AccountLTHash>)>>,
    /// the pubkey, hash pairs that were written by the last slot. MANY accounts
    /// are written EVERY slot. This avoids a re-hashing.
...
impl Bank {
    ...
    fn hash_internal_state(&self) -> Hash {
        ...
        // *h is 32bytes different from LT hash 1024 byte. We won't be able to reuse it here!
        // TODO: revisit this optimization in future.
