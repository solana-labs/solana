...
pub struct BankForks {
    ...
    transaction_runner: Arc<std::sync::Mutex<SchedulerPool>>,
}
...
impl BankForks {
    ...
    pub fn new_from_banks(initial_forks: &[Arc<Bank>], root: Slot) -> Self {
        ...
        Self {
            ...
            transaction_runner: Arc::new(std::sync::Mutex::new(crate::bank::SchedulerPool::new()))
        }
    }
    ...
    fn add_new_bank(&mut self, bank: Bank, mode: solana_scheduler::Mode) -> Arc<Bank> {
        ...
        bank.install_scheduler(self.transaction_runner.take_scheduler_from_pool());
