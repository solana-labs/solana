...
pub struct BankForks {
    ...
    like_scheduler_pool: Option<Box<dyn crate::bank::LikePool>>,
}
...
impl BankForks {
    ...
    fn add_new_bank(&mut self, bank: Bank, mode: solana_scheduler::Mode) -> Arc<Bank> {
        ...
        bank.install_scheduler(self.like_scheduler_pool.as_ref().unwrap().take_from_pool(self.like_scheduler_pool.as_ref().unwrap().clone(), mode));
        bank
    }
