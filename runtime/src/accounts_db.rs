...
pub mod tests {
    ...
    fn test_calculate_accounts_hash_check_hash_mismatch() {
        ...
        db.store_accounts_unfrozen(some_slot, &[(&key, &account)], Some(&[&Hash::default()]), false);
        db.add_root(some_slot);
        let check_hash = true;
        assert!(db.calculate_accounts_hash_helper(false, some_slot, &ancestors, check_hash).is_err());
        assert!(db.calculate_accounts_hash_helper(true, some_slot, &ancestors, check_hash).is_err());
    }
    ...
    fn test_calculate_accounts_hash_check_hash() {
        ...
        let check_hash = true;
        assert_eq!(
            db.calculate_accounts_hash_helper(false, some_slot, &ancestors, check_hash).unwrap(),
