...
use crate::{
    accounts_index::{
        AccountIndex, AccountsIndex, Ancestors, IndexKey, SlotList, SlotSlice, ZeroLamport,
    },
...
pub struct AccountsDB {
    ...
    /// Set of unique keys per updated slot which is used
...
impl AccountsDB {
    ...
    fn construct_candidate_clean_keys(
        ...
        let mut collect_delta_keys = Measure::start("collect_delta_keys");
        let delta_keys: Vec<_> = {
            ...
            slots
                ...
                .filter_map(|slot| {
                    ...
                    if self.accounts_index.is_root(slot) {
                        ...
                        let (_slot, keys) =
                            maybe_slot_keys.expect("rooted slots should be present");
                        Some(keys)
                    } else {
                        None
                    }
        ...
        let mut hashset_to_vec = Measure::start("hashset_to_vec");
        let pubkeys: Vec<Pubkey> = pubkeys.into_iter().collect();
        hashset_to_vec.stop();
        timings.hashset_to_vec_us += hashset_to_vec.as_us();

        timings.delta_key_count = pubkeys.len() as u64 - timings.zero_lamport_count;
    ...
    pub fn clean_accounts(&self, max_clean_root: Option<Slot>) {
                        ...
                        for pubkey in pubkeys {
                                ...
                                self.accounts_index.get(pubkey, None, max_clean_root)
                            {
                                ...
                                let has_zero_lamport_accounts = slot_list
                                    .iter()
                                    .any(|(_slot, account_info)| account_info.is_zero_lamport());
        ...
        datapoint_info!(
            ...
            ("hashset_to_vec_us", key_timings.hashset_to_vec_us, i64),
            ("accounts_scan_us", accounts_scan.as_us() as i64, i64),
            ("store_counts_us", store_counts_time.as_us() as i64, i64),
            ("purge_filter_us", purge_filter.as_us() as i64, i64),
            ("calc_deps_us", calc_deps_time.as_us() as i64, i64),
            ("handle_reclaims_us", reclaims_time.as_us() as i64, i64),
    ...
    fn purge_slots(&self, slots: &HashSet<Slot>) {
        ...
        for slot in non_roots {
            ...
            self.uncleaned_pubkeys.remove(slot);
        }
...
pub mod tests {
    ...
    fn test_partial_clean() {
        ...
        db.store(0, &[(&account_key1, &account1)]);
        db.store(0, &[(&account_key2, &account1)]);
        db.store(1, &[(&account_key1, &account2)]);
        db.get_accounts_delta_hash(0);
        db.get_accounts_delta_hash(1);
        ...
        // store into slot 2
        db.store(2, &[(&account_key2, &account3)]);
        db.store(2, &[(&account_key1, &account3)]);
        ...
        db.store(3, &[(&account_key2, &account4)]);
