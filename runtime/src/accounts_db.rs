...
99705
99701
9970 failed - trying again - trying again - passed again
9965 succeed
9964 succeed
99639
99635 succeed
...
9962 failed
9960 failed - failed again and again
9955 failed
9953 failed
9952 failed
9951 failed
9950 failed
        */
        let epoch_width = DEFAULT_SLOTS_PER_EPOCH * 996305 / 1000000; // put some 'in-this-epoch' slots into an ancient append vec
    ...
    fn combine_ancient_slots(&self, sorted_slots: Vec<Slot>, max_root: Slot) {
        ...
        for slot in sorted_slots {
            ...
            if let Some(storages) = self.storage.map.get(&slot) {
                ...
                drop(read);
                let size = Self::get_ancient_append_vec_capacity();
