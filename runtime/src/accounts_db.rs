...
impl AccountsDB {
    ...
    fn de_dup_and_eliminate_zeros(
        ...
        let hashes: Vec<Vec<Vec<Hash>>> = sorted_data_by_pubkey
            .into_iter()
            .map(|pubkey_division| {
                let (hashes, sum) = Self::de_dup_accounts_in_parallel(&pubkey_division);
    ...
    fn rest_of_hash_calculation(
        accounts: (Vec<Vec<Vec<CalculateHashIntermediate>>>, Measure),
        bins: usize,
    ) -> (Hash, u64) {
    ...
    fn scan_snapshot_stores(
        ...
        simple_capitalization_enabled: bool,
        bins: usize,
    ) -> (Vec<Vec<Vec<CalculateHashIntermediate>>>, Measure) {
...
pub mod tests {
    ...
    fn test_accountsdb_rest_of_hash_calculation() {
        ...
        let result = AccountsDB::rest_of_hash_calculation(
            (vec![vec![account_maps.clone()]], Measure::start("")),
            BINS,
        );
        ...
        let result = AccountsDB::rest_of_hash_calculation(
            (vec![vec![account_maps]], Measure::start("")),
            BINS,
        );
