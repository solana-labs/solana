...
impl<'a> TypeContext<'a> for Context {
    ...
    {
        error!("deserializing");
        let (bank_fields, mut accounts_db_fields) = Self::deserialize_bank_fields(stream_read).unwrap();
        error!("deserialized, hash: {:?}, slot: {}", accounts_db_fields.3.snapshot_hash, bank_fields.slot);
        accounts_db_fields.3.snapshot_hash = *hash;
        let rhs = bank_fields;
        ...
        error!("serialize into");

        let r = bincode::serialize_into(stream_write, 
