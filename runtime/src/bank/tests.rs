...
use {
    super::{
        test_utils::{goto_end_of_slot_without_scheduler, update_vote_account_timestamp},
        *,
    },
    crate::{
...
pub fn create_simple_test_bank(lamports: u64) -> Bank {
...
fn get_shrink_account_size() -> usize {
    ...
    bank0.restore_old_behavior_for_fragile_tests();
    goto_end_of_slot_without_scheduler(&bank0);
    bank0.freeze();
    bank0.squash();
    add_root_and_flush_write_cache(&bank0);
...
fn test_program_execution_restricted_for_stake_account_in_reward_period() {
    ...
    for slot in 1..=num_slots_in_epoch + 2 {
        ...
        bank.register_recent_blockhash(&Hash::new_unique(), &BankWithScheduler::no_scheduler_available());
