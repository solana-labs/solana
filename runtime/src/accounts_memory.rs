use crate::accounts::ErrorCounters;
...
struct Accounts {
    accounts_db: RwLock<AccountsDB>,

    /// set of accounts which are currently in the pipeline
    account_locks: Mutex<HashSet<Pubkey>>,

    /// List of all parents corresponding to this fork
    parents: RwLock<Option<Arc<Accounts>>>,
}

pub struct HashMapAccountsStore {
    accounts: RwLock<HashMap<Fork, Arc<Accounts>>>,
}
...
impl AccountsStore for HashMapAccountsStore {
    fn parent(&self) -> Option<Arc<Accounts>> {
    ...
    parents: RwLock<Option<Arc<Accounts>>>,
    accounts: RwLock<HashMap<Fork, Arc<Accounts>>>,
        let accounts = self.accounts.read().unwrap().get(&fork).unwrap();
        let mut dbs = vec![&account_info.accounts_db.read().unwrap()];
        let mut db_parents = vec![];
        let mut accounts_parent = accounts.parent();
        ...
        let db1: Vec<_> = account_info
            .parents
            .iter()
            .map(|obj| &obj.accounts_db)
            .collect();
        dbs.extend(&db1);
        return AccountsDB::load(&dbs, pubkey);
    }
    ...
    pub fn store_slow(&self, fork: Fork, purge: bool, pubkey: &Pubkey, account: &Account) {
        let info = self.accounts.read().unwrap().get(&fork).unwrap();
        info.accounts
            .write()
            .unwrap()
            .accounts_db
    ...
    pub fn hash_internal_state(&self, fork: Fork) -> Hash {
        let info = self.accounts.read().unwrap().get(&fork).unwrap();
        info.accounts
            .read()
            .unwrap()
    ...
    pub fn lock_accounts(&self, fork: Fork, txs: &[Transaction]) -> Vec<Result<()>> {
        let locks = self.account_locks.read().unwrap().get(&fork).unwrap();
        let mut account_locks = locks.lock().unwrap();
    ...
    pub fn load_accounts(
        ...
        let info = self.accounts.read().unwrap().get(&fork).unwrap();
        let account_info = info.read().unwrap();
        let mut dbs = vec![&account_info.accounts_db];
        let db1: Vec<_> = account_info
            .parents
            .iter()
            .map(|obj| &obj.accounts_db)
