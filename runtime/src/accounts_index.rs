...
impl<'a, T: Clone + std::fmt::Debug + IsCached + Guts> AccountsIndexIterator<'a, T> {
    ...
    fn end_bin_inclusive(&self) -> usize {
        // end in bin where 'start_bound' would exist
...
impl<T: 'static + Clone + IsCached + ZeroLamport + std::marker::Sync + std::marker::Send + std::fmt::Debug + Guts>
    AccountsIndex<T>
{
    /*
    pub fn flush(&self) {
