use crate::{
    ...
    contains::Contains,
    hybrid_btree_map::{
        HybridBTreeMap, HybridEntry as Entry, HybridOccupiedEntry, HybridVacantEntry,
    },
...
pub trait IsCached: 'static + Clone + Debug + PartialEq + ZeroLamport + Sync + Send + Copy {
...
impl<'a, T: Clone + std::fmt::Debug + IsCached> ReadAccountMapEntry<'a, T> {
...
#[self_referencing]
pub struct WriteAccountMapEntry<'a, 'b: 'a, T: IsCached> {
...
impl<'a, 'b: 'a, T: IsCached> WriteAccountMapEntry<'a, 'b, T> {
    pub fn new_with_lock(
        pubkey: &'a Pubkey,
        lock: AccountMapsWriteLock<'b, T>,
        allow_vacant: bool,
    ...
    pub fn get(&'a self) -> Option<&'a HybridOccupiedEntry<'a, T>> {
    ...
    pub fn upsert(
        ...
        w_account_maps.upsert(pubkey, new_value, reclaims, previous_slot_entry_was_cached);
    }
    ...
    pub fn update(&mut self, slot: Slot, account_info: T, reclaims: &mut SlotList<T>) {
        self.slot_list_mut(|list, refcount| {
            if Self::update_slot_list(list, slot, account_info, reclaims, false) {
                *refcount += 1;
            }
        });
    }
...
pub struct AccountsIndexIterator<'a, T: 'static + Clone + IsCached + std::fmt::Debug> {
...
impl<'a, T: IsCached> Iterator for AccountsIndexIterator<'a, T> {
    type Item = Vec<(Pubkey, SlotList<T>)>;
...
#[derive(Debug)]
pub struct AccountsIndex<T: 'static + Clone + std::fmt::Debug + IsCached> {
...
impl<T: IsCached> Drop for AccountsIndex<T> {
    fn drop(&mut self) {
...
impl<T: IsCached> AccountsIndex<T> {
    ...
    pub fn new(bins: usize) -> Self {
        let (account_maps, bin_calculator, exit, flusher) = Self::allocate_accounts_index(bins);
        Self {
            account_maps,
            bin_calculator,
        ...
        R: RangeBounds<Pubkey> + Debug,
    {
        ...
        for pubkey_list in self.iter(range, all_and_unsorted) {
            ...
            for (pubkey, slot_list) in pubkey_list {
                //error!("pubkey: {}", pubkey);
                num_keys_iterated += 1;
                let ix = self.latest_slot(Some(ancestors), &slot_list[..], max_root);
                if let Some(ix) = ix {
                    let result = slot_list[ix];
    ...
    pub(crate) fn insert_new_if_missing_into_primary_index(
        ...
        let dirty_pubkeys = binned
            ...
            .map(|(pubkey_bin, items)| {
                ...
                for (pubkey, new_item, is_zero_lamport) in items.into_iter() {
                    let (already_exists, returned_lock) =
                        self.upsert_with_lock_pubkey_result(pubkey, w_account_maps, new_item);
    ...
    pub fn purge_roots(&self, pubkey: &Pubkey) -> (SlotList<T>, bool) {
        ...
        write_account_map_entry
            .slot_list_mut(|slot_list: &mut SlotList<T>, _ref_count| {
...
pub mod tests {
        ...
        loop {
            ...
            (0..threads).into_par_iter().for_each(|_| {
                let data = (0..count)
                    ...
                    .collect::<Vec<_>>();
                index.insert_new_if_missing_into_primary_index(0, data.len(), data.into_iter());
            });
            m0.stop();
            ...
            let cache_entries = (0..threads)
                ...
                .map(|_| {
                    ...
                    loop {
                        ...
                        size += index.account_maps[old].read().unwrap().flush();
                    }
                    size
                })
                .sum::<usize>();
    ...
    fn account_maps_len_expensive<T: IsCached>(index: &AccountsIndex<T>) -> usize {
        index
            ...
            .iter()
            .map(|bin_map| {
                let read = bin_map.read().unwrap();
                read.flush();
                read.range(None::<Range<Pubkey>>).len()
            })
            .sum()
    }
    ...
    >(
        ...
        index
            ...
            .unwrap()
            .slot_list_mut(|slot_list, _ref_count| slot_list.clear());
