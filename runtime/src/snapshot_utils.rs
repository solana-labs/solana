...
/// A slot and the path to its snapshot
#[derive(PartialEq, Eq, Debug)]
pub struct SlotSnapshotPath {
    pub slot: Slot,
...
pub fn package_snapshot<P, Q>(
    bank: &Bank,
    slot_snapshot_path: &SlotSnapshotPath,
    snapshots_dir: Q,
    ...
    P: AsRef<Path>,
{
    ...
    // Hard link the snapshot into a tmpdir, to ensure its not removed prior to packaging.
    {
        let snapshot_hardlink_dir = snapshot_tmpdir
            .as_ref()
            .join(slot_snapshot_path.slot.to_string());
        fs::create_dir_all(&snapshot_hardlink_dir)?;
        fs::hard_link(
            &slot_snapshot_path.snapshot_path,
            &snapshot_hardlink_dir.join(slot_snapshot_path.slot.to_string()),
        )?;
    }
...
pub fn remove_tmp_snapshot_archives(snapshots_dir: &Path) {
...
/// Get a list of SlotSnapshotPath for the given snapshots directory
pub fn get_slot_snapshot_paths<P>(snapshots_dir: P) -> Vec<SlotSnapshotPath>
where
    P: AsRef<Path>,
{
    match fs::read_dir(&snapshots_dir) {
        Ok(paths) => paths
            ...
            .map(|slot| {
                ...
                // Yep, you read that right, the same snapshot file name is joined twice in a row.
                // "Why" you ask?  This I do not know.
...
/// Get a list of SlotSnapshotPath for the given snapshot directory, sorted by slot
pub fn get_sorted_slot_snapshot_paths<P>(snapshots_dir: P) -> Vec<SlotSnapshotPath>
where
    P: AsRef<Path>,
{
    let mut slot_snapshot_paths = get_slot_snapshot_paths(snapshots_dir);
    slot_snapshot_paths.sort_unstable_by(|a, b| a.slot.cmp(&b.slot));
    slot_snapshot_paths
}
...
pub fn deserialize_snapshot_data_file<T: Sized>(
    ...
    deserialize_snapshot_data_file_capped(data_file_path, MAX_SNAPSHOT_DATA_FILE_SIZE, deserializer)
}
...
fn deserialize_snapshot_data_file_capped<T: Sized>(
    data_file_path: &Path,
    maximum_file_size: u64,
    deserializer: impl FnOnce(&mut BufReader<File>) -> Result<T>,
) -> Result<T> {
...
pub fn add_snapshot<P: AsRef<Path>>(
    ...
    snapshot_version: SnapshotVersion,
) -> Result<SlotSnapshotPath> {
    let slot = bank.slot();
    // snapshots_dir/slot
    let slot_snapshot_dir = get_bank_snapshot_dir(snapshots_dir, slot);
...
pub fn remove_snapshot<P>(slot: Slot, snapshots_dir: P) -> Result<()>
where
    P: AsRef<Path>,
{
    let slot_snapshot_dir = get_bank_snapshot_dir(&snapshots_dir, slot);
...
where
    ...
    Q: AsRef<Path>,
{
    fn path_to_file_name(path: &Path) -> Result<&str> {
        path.file_name()
    ...
    let full_snapshot_filename = path_to_file_name(full_snapshot_archive_path.as_ref())?;
    let (full_snapshot_slot, _, _) = parse_snapshot_archive_filename(full_snapshot_filename)
        .ok_or_else(|| {
            SnapshotError::ParseSnapshotArchiveFileNameError(full_snapshot_filename.to_string())
        })?;

    let incremental_snapshot_filename =
        path_to_file_name(incremental_snapshot_archive_path.as_ref())?;
    let (incremental_snapshot_base_slot, _, _, _) = parse_incremental_snapshot_archive_filename(
        incremental_snapshot_filename,
    )
    .ok_or_else(|| {
        SnapshotError::ParseSnapshotArchiveFileNameError(incremental_snapshot_filename.to_string())
    })?;
        ...
        Ok(files) => files
            .filter_map(|entry| {
                if let Ok(entry) = entry {
                    let path = entry.path();
                    if path.is_file() {
                        if let Some((base_slot, slot, hash, archive_format)) =
...
/// Sort the list of incremental snapshot archives, first by base (full snapshot) slot in
/// descending order, then by "regular" (incremental snapshot) slot in descending order
fn sort_incremental_snapshot_archives(
    incremental_snapshot_archives: &mut Vec<IncrementalSnapshotArchiveInfo>,
) {
    incremental_snapshot_archives.sort_unstable_by(|a, b| {
        b.base_slot
            .cmp(&a.base_slot)
            .then(b.inner.slot.cmp(&a.inner.slot))
    });
}
...
pub fn get_highest_incremental_snapshot_archive_slot<P: AsRef<Path>>(
    ...
    get_highest_incremental_snapshot_archive_info(snapshot_output_dir, full_snapshot_slot)
        .map(|incremental_snapshot_archive_info| incremental_snapshot_archive_info.inner.slot)
}
    ...
    get_incremental_snapshot_archives(snapshot_output_dir.as_ref())
        ...
        .for_each(|old_archive| {
            trace!(
                ...
                old_archive.inner.path.display()
            );
            fs::remove_file(old_archive.inner.path.as_path())
...
fn verify_unpacked_snapshots_dir_and_version(
...
) -> Result<(SnapshotVersion, SlotSnapshotPath)> {
    ...
    let mut slot_snapshot_paths =
        get_sorted_slot_snapshot_paths(&unpacked_snapshots_dir_and_version.unpacked_snapshots_dir);
    if slot_snapshot_paths.len() > 1 {
        return Err(get_io_error("invalid snapshot format"));
    }
    let root_paths = slot_snapshot_paths
        .pop()
...
fn rebuild_bank_from_snapshots(
    ...
    let (incremental_snapshot_version, incremental_snapshot_root_paths) =
        if let Some(snapshot_unpacked_snapshots_dir_and_version) =
            incremental_snapshot_unpacked_snapshots_dir_and_version
        {
            let (snapshot_version, slot_snapshot_path) = verify_unpacked_snapshots_dir_and_version(
                snapshot_unpacked_snapshots_dir_and_version,
            )?;
            (Some(snapshot_version), Some(slot_snapshot_path))
        } else {
            (None, None)
        };
    info!(
...
/// Remove outdataed snapshots
pub fn purge_old_snapshots<P>(snapshots_dir: P)
where
    P: AsRef<Path>,
{
    let slot_snapshot_paths = get_sorted_slot_snapshot_paths(&snapshots_dir);
    let num_to_remove = slot_snapshot_paths.len().saturating_sub(MAX_SNAPSHOTS);
    for slot_snapshot_path in &slot_snapshot_paths[..num_to_remove] {
        let r = remove_snapshot(slot_snapshot_path.slot, &snapshots_dir);
        if r.is_err() {
            warn!(
                "Couldn't remove snapshot at: {}",
                slot_snapshot_path.snapshot_path.display()
            );
        }
    }
}
...
pub fn snapshot_bank(
    ...
    add_snapshot(snapshots_dir, root_bank, &storages, snapshot_version)?;
...
pub fn bank_to_snapshot_archive<P: AsRef<Path>, Q: AsRef<Path>>(
    ...
    let slot_snapshot_path = add_snapshot(&temp_dir, bank, &storages, snapshot_version)?;
    let package = package_snapshot(
        bank,
        &slot_snapshot_path,
        &temp_dir,
...
mod tests {
    ...
    fn test_get_sorted_incremental_snapshot_archives() {
        ...
        assert_eq!(
            sorted_incremental_snapshot_archives[0].inner.slot,
            max_incremental_snapshot_slot - 1
        );
    }
    ...
    fn test_purge_old_incremental_snapshot_archives() {
        ...
        for snapshot_filename in [
            ...
            let snapshot_path = snapshots_dir.path().join(&snapshot_filename);
            File::create(snapshot_path).unwrap();
        }

        purge_old_snapshot_archives(snapshots_dir.path(), std::usize::MAX);

        let remaining_incremental_snapshot_archives =
            get_sorted_incremental_snapshot_archives(snapshots_dir.path());
