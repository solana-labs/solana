...
fn parse_accounts_file(account_path: &OsString, account_vec: &mut Vec<Accounts>) -> Result<(), Box<dyn Error>> {
    ...
    for result in rdr.records() {
        ...
        let pubkey: Pubkey = Pubkey::from_str( &record[0]
            .to_string())
            .unwrap();
        let stake: u64 = record[1]
            .to_string()
            .trim()
            .parse::<u64>()
            .unwrap();
...
pub fn main() {
    ...
    for account in accounts {
        println!("account: {}, {}", account.pubkey, account.stake);
    }
    ...
    println!("keys: {}, {}, {}", node_keypair_0.pubkey(), node_keypair_1.pubkey(), node_keypair_2.pubkey());
    println!("suhh");
