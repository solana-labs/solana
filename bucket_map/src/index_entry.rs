        ...
        Some((
            match self.num_slots {
                ...
                1 => {
                    ...
                    std::slice::from_ref(&self.first_element)
                }
                0 => {
...
mod tests {
    ...
    impl<T: Clone + Copy + Debug + Default> IndexEntry<T> {
    ...
    fn test_api() {
        for offset in [0, 1, u32::MAX as u64] {
            let mut index = IndexEntry::<u64>::new(solana_sdk::pubkey::new_rand());
            if offset != 0 {
    ...
    fn test_set_storage_offset_value_too_large() {
        let too_big = 1 << 56;
        let mut index = IndexEntry::<u64>::new(Pubkey::new_unique());
