...
fn acquire_passphrase_and_message(matches: &ArgMatches<'_>) -> Result<(String, String), Box<dyn error::Error>> {
    if matches.is_present("no_passphrase") {
        ...
        match prompt_passphrase(
            ...
            Ok(passphrase) => { println!(); Ok((passphrase, " and your BIP39 passphrase".to_string())) },
            Err(e) => Err(e)
        }
    }
}
...
fn do_main(matches: &ArgMatches<'_>) -> Result<(), Box<dyn error::Error>> {
    ...
    match matches.subcommand() {
        ...
        ("grind", Some(matches)) => {
            ...
            let (passphrase, passphrase_message) = if use_mnemonic { acquire_passphrase_and_message(matches).unwrap() } else
