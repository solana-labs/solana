...
impl Measure {
    ...
    /// # use solana_measure::measure::Measure;
    /// let (result, measure) = Measure::this(|| std::cmp::min(42, 123), "minimum");
    /// # assert_eq!(result, 42);
    /// ```
    ///
    /// ```
    ...
    /// # let baz = 8;
    /// let foo = Foo { x: 42 };
    /// let (result, measure) = Measure::this(|| Foo::bar(&foo, baz), "Foo::bar");
    /// # assert_eq!(result, 50);
    /// ```
    pub fn this<R, F: FnOnce() -> R>(func: F, name: &'static str) -> (R, Self) {
...
mod tests {
    ...
    fn test_measure_this() {
        ...
        {
            let (_result, measure) = Measure::this(|| sleep(Duration::from_secs(1)), "test");
            assert!(measure.as_s() >= 0.99f32 && measure.as_s() <= 1.01f32);
            assert!(measure.as_ms() >= 990 && measure.as_ms() <= 1_010);
            assert!(measure.as_us() >= 999_000 && measure.as_us() <= 1_010_000);
        }
        ...
        {
            let expected = 1;
            let (actual, _measure) = Measure::this(|| expected, "test");
            assert_eq!(actual, expected);
        }

        // Ensure that this() can be called with a normal function
        {
            let (result, _measure) = Measure::this(|| my_multiply(3, 4), "test");
        ...
        // Ensure that this() can be called with a method (and self)
        {
            ...
            let (result, _measure) = Measure::this(|| SomeStruct::add_to(&some_struct, 4), "test");
