...
fn hardforks_of(matches: &ArgMatches, name: &str) -> Option<Vec<Slot>> {
    if matches.is_present(name) {
        Some(matches.values_of_t_or_exit::<Slot>(name))
    } else {
        None
    }
}
...
fn main() {
    ...
    // move here to surpress error temporary value dropped while borrowed
    ...
    let disable_disk_index = Arg::new("disable_accounts_disk_index")
    ...
    let accounts_filler_size = Arg::new("accounts_filler_size")
    ...
    let no_os_memory_stats_reporting_arg = Arg::new("no_os_memory_stats_reporting")
        .long("no-os-memory-stats-reporting")
        .help("Disable reporting of OS memory statistics.");
    let skip_rewrites_arg = Arg::new("accounts_db_skip_rewrites")
    ...
    let accounts_db_skip_initial_hash_calc_arg =
        Arg::new("accounts_db_skip_initial_hash_calculation")
            ...
            .hide(true);
    let ancient_append_vecs = Arg::new("accounts_db_ancient_append_vecs")
    ...
    let usage: String = command.render_usage();
    let matches = command.get_matches();

    info!("{} {}", crate_name!(), solana_version::version!());

    let ledger_path = parse_ledger_path(&matches, &usage, "ledger_path");

    let snapshot_archive_path = matches
        .value_of_t::<String>("snapshot_archive_path")
        .ok()
        .map(PathBuf::from);
    let incremental_snapshot_archive_path = matches
        .value_of_t::<String>("incremental_snapshot_archive_path")
        .ok()
        .map(PathBuf::from);
    ...
    if let ("bigtable", arg_matches) = matches.subcommand().unwrap() {
        ...
        match subcommand {
            ("print", arg_matches) => {
                ...
                let ending_slot: Slot = arg_matches.value_of_t("ending_slot").unwrap_or(Slot::MAX);
                let num_slots = arg_matches.value_of_t::<Slot>("num_slots").ok();
            ...
            ("copy", arg_matches) => {
                let starting_slot: Slot = arg_matches.value_of_t_or_exit("starting_slot");
                let ending_slot: Slot = arg_matches.value_of_t_or_exit("ending_slot");
                let target_db =
                    PathBuf::from(arg_matches.value_of_t_or_exit::<String>("target_db"));
            ...
            ("modify-genesis", arg_matches) => {
                ...
                if let Some(hashes_per_tick) = arg_matches.value_of("hashes_per_tick") {
                    genesis_config.poh_config.hashes_per_tick = match hashes_per_tick {
                        ...
                        "sleep" => None,
                        _ => Some(arg_matches.value_of_t_or_exit::<u64>("hashes_per_tick")),
                    }
                }
            ...
            ("parse_full_frozen", arg_matches) => {
                ...
                let log_file = PathBuf::from(arg_matches.value_of_t_or_exit::<String>("log_path"));
            ...
            ("verify", arg_matches) => {
...

                {
                    let mut accounts_index_paths: Vec<PathBuf> =
                        if arg_matches.is_present("accounts_index_path") {
                            arg_matches
                                .values_of_t_or_exit::<String>("accounts_index_path")
                                .into_iter()
                                .map(PathBuf::from)
                                .collect()
                        } else {
                ...
                let filler_accounts_config = FillerAccountsConfig {
                    count: arg_matches.value_of_t_or_exit::<usize>("accounts_filler_count"),
                    size: arg_matches.value_of_t_or_exit::<usize>("accounts_filler_size"),
                };
                ...
                let process_options = ProcessOptions {
                    ...
                    halt_at_slot: arg_matches.value_of_t::<Slot>("halt_at_slot").ok(),
                    debug_keys,
                    accounts_db_caching_enabled: !arg_matches.is_present("no_accounts_db_caching"),
                    limit_load_slot_count_from_snapshot: arg_matches
                        .value_of_t::<usize>("limit_load_slot_count_from_snapshot")
                        .ok(),
                    accounts_db_config,
            ...
            ("graph", arg_matches) => {
                let output_file: String = arg_matches.value_of_t_or_exit("graph_filename");
            ...
            ("create-snapshot", arg_matches) => {
                ...
                let mut warp_slot = arg_matches.value_of_t::<Slot>("warp_slot").ok();
                ...
                let faucet_lamports = arg_matches
                    .value_of_t::<u64>("faucet_lamports")
                    .unwrap_or(0);

                let rent_burn_percentage = arg_matches.value_of_t::<u8>("rent_burn_percentage");
                ...
                let bootstrap_validator_lamports: u64 =
                    arg_matches.value_of_t_or_exit("bootstrap_validator_lamports");
                let bootstrap_validator_stake_lamports: u64 =
                    arg_matches.value_of_t_or_exit("bootstrap_validator_stake_lamports");
                ...
                let snapshot_archive_format = {
                    let archive_format_str: String =
                        arg_matches.value_of_t_or_exit("snapshot_archive_format");
                    ArchiveFormat::from_cli_arg(&archive_format_str).unwrap_or_else(|| {
                        panic!("Archive format not recognized: {}", archive_format_str)
                    })
                };

                let maximum_full_snapshot_archives_to_retain: usize =
                    arg_matches.value_of_t_or_exit("maximum_full_snapshots_to_retain");
                let maximum_incremental_snapshot_archives_to_retain: usize =
                    arg_matches.value_of_t_or_exit("maximum_incremental_snapshots_to_retain");
                let genesis_config = open_genesis_config_by(&ledger_path, arg_matches);
                let blockstore =
                    open_blockstore(&ledger_path, AccessType::Secondary, wal_recovery_mode);

                let snapshot_slot: Slot = if Some("ROOT") == arg_matches.value_of("snapshot_slot") {
                    blockstore
                    ...
                    arg_matches.value_of_t_or_exit("snapshot_slot")
                };
                        ...
                        if child_bank_required {
                            ...
                            if let Some(hashes_per_tick) = hashes_per_tick {
                                child_bank.set_hashes_per_tick(match hashes_per_tick {
                                    ...
                                    "sleep" => None,
                                    _ => Some(
                                        arg_matches.value_of_t_or_exit::<u64>("hashes_per_tick"),
                                    ),
                                });
                            }
            ...
            ("accounts", arg_matches) => {
                let halt_at_slot = arg_matches.value_of_t::<Slot>("halt_at_slot").ok();
                let process_options = ProcessOptions {
                    new_hard_forks: hardforks_of(arg_matches, "hard_forks"),
                    halt_at_slot,
                    poh_verify: false,
                ...
                match load_bank_forks(
                    ...
                    Ok((bank_forks, ..)) => {
                        ...
                        if arg_matches.is_present("warp_epoch") {
                            ...
                            if let Ok(raw_inflation) = arg_matches.value_of_t::<String>("inflation")
                            {
                            ...
                            let mut csv_writer = if arg_matches.is_present("csv_filename") {
                                let csv_filename: String =
                                    arg_matches.value_of_t_or_exit("csv_filename");
            ...
            ("purge", arg_matches) => {
                let start_slot: Slot = arg_matches.value_of_t_or_exit("start_slot");
                let end_slot = arg_matches.value_of_t::<Slot>("end_slot").ok();
                let no_compaction = arg_matches.is_present("no_compaction");
                let dead_slots_only = arg_matches.is_present("dead_slots_only");
                let batch_size: usize = arg_matches.value_of_t_or_exit("batch_size");
            ...
            ("latest-optimistic-slots", arg_matches) => {
                let blockstore =
                    open_blockstore(&ledger_path, AccessType::Secondary, wal_recovery_mode);
                let num_slots: usize = arg_matches.value_of_t_or_exit("num_slots");
            ...
            ("repair-roots", arg_matches) => {
                ...
                let max_slots: u64 = arg_matches.value_of_t_or_exit("max_slots");
            ...
            ("compute-slot-cost", arg_matches) => {
                ...
                if !arg_matches.is_present("slots") {
                    ...
                    slots = arg_matches.values_of_t_or_exit::<Slot>("slots");
                }
