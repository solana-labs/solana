...
pub fn get_accounts_db_config(
    ...
    let test_partitioned_epoch_rewards =
        if arg_matches.is_present("partitioned-epoch-rewards-compare_calculation") {
            TestPartitionedEpochRewards::CompareResults
        } else if arg_matches.is_present("partitioned-epoch-rewards-force-enable-single-slot") {
