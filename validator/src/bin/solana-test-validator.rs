...
fn main() {
    ...
    let matches = App::new("solana-test-validator")
        ...
        .arg(
            Arg::with_name("max_compute_unit_limit")
                .long("max-compute-unit-limit")
                .alias("max-compute-units")
                .value_name("COMPUTE_UNITS")
                .validator(is_parsable::<u64>)
                .takes_value(true)
                .help("Override the runtime's maximum compute unit limit")
        )
        .get_matches();
    ...
    let max_compute_unit_limit = value_t!(matches, "max_compute_unit_limit", u64).ok();
    ...
    if let Some(compute_unit_limit) = max_compute_unit_limit {
        genesis.max_compute_unit_limit(compute_unit_limit);
    }
