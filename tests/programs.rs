...
fn load_program(bank: &Bank, from: &Keypair, loader_id: Pubkey, program: Vec<u8>) -> Pubkey {
    ...
    let tx = Transaction::loader_finalize(&program_account, loader_id, bank.last_id(), 0);
    bank.process_transaction(&tx).unwrap();

    let tx = Transaction::system_spawn(&program_account, bank.last_id(), 0);
    bank.process_transaction(&tx).unwrap();
...
fn test_program_lua_move_funds() {
    ...
    let tx = Transaction::system_create(&mint_keypair, to, bank.last_id(), 1, 0, program_id, 0);
    bank.process_transaction(&tx).unwrap();
