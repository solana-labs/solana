...
use solana::dynamic_instruction::Instruction;
#[cfg(feature = "bpf_c")]
use solana::dynamic_program::{
    DynamicProgram, ProgramError, ProgramPath, PLATFORM_SECTION_C, PLATFORM_SECTION_RS,
};
...
fn test_program_load_native() {
    ...
    let tx = Transaction::program_load_native(&mint.keypair(), program, name, mint.last_id(), 0);
    check_tx_results(&bank, &tx, bank.process_transactions(&vec![tx.clone()]));
    ...
    let tx = Transaction::program_load_state(
        &mint.keypair(),
        state,
        0,
        vec![8u8; 10],
    ...
    let tx = Transaction::program_call(
        &mint.keypair(),
        &vec![program, state],
        vec![1u8],
        mint.last_id(),
...
fn test_program_bpf_move_funds() {
    ...
    let tx = Transaction::system_create(
        ...
        mint.last_id(),
        1,
        mem::size_of_val(&Instruction::LoadBpf {
            offset: 0,
            prog: prog.clone(),
        }) as u64,
        DynamicProgram::id(),
    ...
    // populate program bits account
    let tx = Transaction::program_load_bpf(&mint.keypair(), program, 0, prog, mint.last_id(), 0);
    check_tx_results(&bank, &tx, bank.process_transactions(&vec![tx.clone()]));
