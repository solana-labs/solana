...
fn test_forwarding() {
    ...
    let cluster_nodes = gossip_service::discover(&cluster.entry_point_info.gossip, 2).unwrap();
...
fn test_partition() {
    let fullnode_config = FullnodeConfig::default();
    let (_cluster, partitions) =
        LocalCluster::new_partioned_cluster(&[&[3, 3, 3], &[3, 3]], 2_000_000, &fullnode_config);

    let (spy_gossip, _spy_cluster_info, spy_exit) =
        gossip_service::make_bridge_partition_node(&partitions[..]).unwrap();

    // Cleanup
    spy_exit.store(true, Ordering::Relaxed);
    spy_gossip.join().unwrap();
}
