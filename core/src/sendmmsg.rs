...
mod tests {
    ...
    pub fn test_send_mmsg_one_dest() {
        ...
        let mut datas: Vec<_> = (0..32).map(|_| vec![0u8; PACKET_DATA_SIZE]).collect();
        let mut packets: Vec<_> = datas.iter_mut().map(|d| (&mut d[..], &addr)).collect();
