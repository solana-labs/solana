...
pub struct BroadcastDuplicateBlocksConfig {
    ...
    /// Number of entries to remove from each slot.
    pub num_entries_to_remove: usize,
}

#[derive(Clone)]
pub struct BroadcastDuplicateBlocksRun {
    ...
    prev_entry_hashes: Vec<Option<Hash>>,
}
...
impl BroadcastDuplicateBlocksRun {
    pub(super) fn new(shred_version: u16, config: BroadcastDuplicateBlocksConfig) -> Self {
    ...
    fn process_receive_results(
            ...
            || bank.slot() != self.current_slot_and_parent.unwrap().0
        {
            ...
            self.num_batches = 0;
            self.prev_entry_hashes = vec![None; self.config.num_duplicates];
            let slot = bank.slot();
        ...
        let mut shreds_to_send : Vec<BroadcastShredsData>  = Vec::default();
        ...
        for duplicate_index in 0..self.config.num_duplicates-1 {
            if let Some(last_entry) = new_entries.last() {
                if is_last_in_slot && self.recent_blockhash.is_some() && last_entry.is_tick() {
                    if duplicate_index > 0 {
                        new_entries.pop();
                    } else {
                        new_entries.truncate(receive_results.entries.len().saturating_sub(self.config.num_entries_to_remove));
                    }
                    let prev_entry_hash =
                        // Try to get second-to-last entry before last tick
                        if new_entries.len() > 0 {
                            Some(receive_results.entries[new_entries.len() - 1].hash)
                        } else {
                            self.prev_entry_hashes[duplicate_index]
                        };
                    if duplicate_index > 0 {
                        // need to pop again because we previously replaced the last tick with two entries.
                        new_entries.pop();
                    }
                    ...
                    if let Some(prev_hash) = prev_entry_hash {
                        let new_last_entry = Entry::new(
                            &prev_hash,
                            1,
                            vec![extra_tx],
                        );
                        let new_last_hash = new_last_entry.hash;
                        new_entries.push(new_last_entry);
                        // Adding a tick
        ...
        if let Some(shred) = shreds_to_send[0].data_shreds.first() {
            if shred.index() == 0 {
                blockstore
                    .insert_shreds(
                        vec![shred.clone()],
        ...
        // TODO(wen): We shouldn't need this, investigate why.
...
impl BroadcastRun for BroadcastDuplicateBlocksRun {
    fn run(
        &mut self,
        keypair: &Keypair,
        blockstore: &Blockstore,
...
mod test {
    ...
    fn test_slot_interrupt() {
        ...
        broadcast_duplicate_blocks_run
            ...
            .unwrap();
        let unfinished_slot = broadcast_duplicate_blocks_run.unfinished_slot.as_ref().unwrap();
        ...
        broadcast_duplicate_blocks_run.process_shreds_stats.receive_elapsed = 10;
        ...
        assert_eq!(
            broadcast_duplicate_blocks_run.process_shreds_stats.receive_elapsed,
            0
        );
    ...
    fn test_slot_finish() {
        ...
        broadcast_duplicate_blocks_run
            ...
            .unwrap();
        assert!(broadcast_duplicate_blocks_run.unfinished_slot.is_none())
    }
}
