...
fn prune_shreds_invalid_repair(
    ...
    i = 0;
    parsed_window_shred_metas.retain(|_parsed_window_shred_meta| (!removed.contains(&i), i += 1).0);
    assert_eq!(shreds.len(), parsed_window_shred_metas.len());
}
    ...
    F: Fn(&Shred, Arc<Bank>, /*last root:*/ Slot) -> bool + Sync,
{
    ...
    let handle_packet = |packet: &Packet| {
        ...
        let parsed_window_shred_meta = if packet.meta.repair() {
            ...
            ParsedWindowShredMeta::Turbine(packet.meta.addr())
        };
    ...
    stats.num_repairs += parsed_window_shred_metas
        .iter()
        .filter(|parsed_window_shred_meta| parsed_window_shred_meta.is_repair())
        .count();
...
mod test {
    ...
    fn test_prune_shreds() {
        ...
        let mut parsed_window_shred_metas = vec![
            ParsedWindowShredMeta::Turbine(SocketAddr::new()),
            ParsedWindowShredMeta::RepairMeta(repair_meta),
            ParsedWindowShredMeta::RepairMeta(repair_meta1),
        ];
        prune_shreds_invalid_repair(
            &mut shreds,
            &mut parsed_window_shred_metas,
        ...
        assert_eq!(parsed_window_shred_metas.len(), 2);
        assert!(parsed_window_shred_metas[0].is_none());
        assert_eq!(parsed_window_shred_metas[1].as_ref().unwrap().nonce, nonce);
    }
}
