...
fn retransmit_blobs(blobs: &[Blob], retransmit: &BlobSender, id: &Pubkey) -> Result<()> {
    let mut retransmit_queue: Vec<Blob> = Vec::new();
    for blob in blobs {
...
fn process_blobs(blobs: &[Blob], blocktree: &Arc<Blocktree>) -> Result<()> {
    // make an iterator for insert_data_blobs()
    let blobs: Vec<_> = blobs.iter().map(move |blob| blob).collect();
...
mod test {
    ...
    fn test_process_blob() {
        ...
        let mut blobs = original_entries.clone().to_blobs();

        index_blobs(&mut blobs, &Pubkey::new_rand(), 0, 0, 0);
