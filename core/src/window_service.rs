...
impl ReceiveWindowStats {
    fn maybe_submit(&mut self) {
        ...
        datapoint_info!(
            ...
            ("num_packet_batches", self.num_packet_batches, i64),
            ("num_coalesced_batches", self.num_coalesced_batches, i64),
...
fn run_insert<F>(
    shred_receiver: &CrossbeamReceiver<(Shreds, Vec<Option<RepairMeta>>)>,
    blockstore: &Blockstore,
    leader_schedule_cache: &LeaderScheduleCache,
    handle_duplicate: F,
...
impl WindowService {
    ...
    fn start_recv_window_thread<F>(
        ...
        exit: Arc<AtomicBool>,
        blockstore: Arc<Blockstore>,
        insert_sender: CrossbeamSender<(Shreds, Vec<Option<RepairMeta>>)>,
        verified_receiver: CrossbeamReceiver<Vec<Packets>>,
        shred_filter: F,
