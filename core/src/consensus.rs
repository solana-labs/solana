...
impl Tower {
    ...
    pub(crate) fn collect_vote_lockouts(
        ...
        for (&key, (voted_stake, account)) in vote_accounts.iter() {
            ...
            let mut vote_state = account.vote_state().as_ref().clone();
    ...
    pub fn last_voted_slot_in_bank(bank: &Bank, vote_account_pubkey: &Pubkey) -> Option<Slot> {
        ...
        vote_account.vote_state().last_voted_slot()
    }
    ...
    fn initialize_lockouts_from_bank(
        ...
        if let Some(vote_account) = bank.get_vote_account(vote_account_pubkey) {
            self.vote_state = vote_account.vote_state().as_ref().clone();
...
pub mod test {
    ...
    fn test_double_partition() {
        ...
        let observed = vote_simulator
            ...
            .unwrap();
        let vote_state = observed.vote_state();
        info!("observed tower: {:#?}", vote_state.votes);
