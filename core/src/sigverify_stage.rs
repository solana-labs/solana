...
use {
    ...
    solana_perf::{
        ...
        sigverify::{count_packets_in_batches, count_valid_packets, shrink_batches, Deduper},
    },
    solana_sdk::timing,
...
const MAX_DISCARDED_PACKET_PCT: usize = 10;
...
impl SigVerifyStage {
    ...
    pub fn maybe_shrink_batches(
        ...
        let num_discarded_packets = num_packets_in_batches.saturating_sub(num_valid_packets);

        // if (num_discarded_packets / num_packets_in_batches) > (MAX_DISCARDED_PACKET_PCT / 100)
        if num_discarded_packets.saturating_mul(100)
            > MAX_DISCARDED_PACKET_PCT.saturating_mul(num_packets_in_batches)
        {
    ...
    fn verifier<T: SigVerifier>(
        ...
        let (mut batches, num_recv_packets, recv_duration) =
        ...
        let num_discarded_randomly = num_recv_packets.saturating_sub(non_discarded_packets);
        discard_random_time.stop();

        let mut dedup_time = Measure::start("sigverify_dedup_time");
        let num_packets_to_dedup = non_discarded_packets;
        ...
        let excess_fail = num_packets_to_ip_sample.saturating_sub(MAX_SIGVERIFY_BATCH);
        discard_time.stop();

        // Pre-shrink packet batches if many packets are discarded from dedup / discard
        let (pre_shrink_time_us, pre_shrink_removed_batches) = {
            let num_packets_in_batches = num_unique;
            let num_valid_packets = num_packets_to_verify;
            Self::maybe_shrink_batches(&mut batches, num_packets_in_batches, num_valid_packets)
        };
        ...
        let (post_shrink_time_us, post_shrink_removed_batches) = {
            let num_packets_in_batches = count_packets_in_batches(&batches);
            Self::maybe_shrink_batches(&mut batches, num_packets_in_batches, num_valid_packets)
        };
        ...
        debug!(
            ...
            verify_time.as_ms(),
            num_recv_packets,
            (num_recv_packets as f32 / verify_time.as_s())
        );
        ...
        stats
            ...
            .increment(discard_time.as_us() / (num_packets_to_ip_sample as u64))
            .unwrap();
        stats
            .dedup_packets_pp_us_hist
            .increment(dedup_time.as_us() / (num_packets_to_dedup as u64))
            .unwrap();
        stats.batches_hist.increment(batches_len as u64).unwrap();
        stats
            .packets_hist
            .increment(num_recv_packets as u64)
            .unwrap();
        stats.total_batches += batches_len;
        stats.total_packets += num_recv_packets;
        ...
        stats.total_shrinks += pre_shrink_removed_batches + post_shrink_removed_batches;
...
mod tests {
    ...
    fn test_maybe_shrink_batches() {
        let packets_per_batch = 128;
        let num_packets = 4096;
        let mut batches = gen_batches(true, packets_per_batch, num_packets);
        let num_generated_batches = batches.len();
        assert_eq!(
            SigVerifyStage::maybe_shrink_batches(&mut batches, num_packets, num_packets).1,
            0
        );
        ...
        // discard until the threshold is met but not exceeded
        {
            ...
            batches.iter_mut().for_each(|batch| {
                batch.iter_mut().for_each(|p| {
                    if ((index + 1) as f64 / num_packets as f64)
                        < MAX_DISCARDED_PACKET_PCT as f64 / 100.0
                    {
        ...
        let mut num_valid_packets = count_valid_packets(&batches, |_| {});
        assert_eq!(
            SigVerifyStage::maybe_shrink_batches(&mut batches, num_packets, num_valid_packets).1,
            0
        );
        ...
        let expected_num_shrunk_batches =
            1.max(((num_generated_batches * MAX_DISCARDED_PACKET_PCT) as f64 / 100.0) as usize);
        num_valid_packets -= 1;
        assert_eq!(
            SigVerifyStage::maybe_shrink_batches(&mut batches, num_packets, num_valid_packets).1,
