...
impl CrdsGossipPull {
    ...
    pub fn filter_pull_responses(
        ...
        let mut versioned_no_timestamp = vec![];
        for r in responses {
                    ...
                    _ => {
                        ...
                        if crds.lookup(&CrdsValueLabel::ContactInfo(owner)).is_none() {
                            ...
                            match crds.would_insert(r, now) {
                                Some(resp) => versioned_no_timestamp.push(resp),
                                None => stats.failed += 1,
                            }
                            continue;
                        }
        ...
        (versioned, versioned_no_timestamp)
    }
    ...
    pub fn process_pull_responses(
        ...
        for r in responses_no_timeout {
            stats.failed += crds.insert_versioned(r).is_err() as usize;
        }
        for r in responses {
    ...
    pub fn process_pull_response(
        ...
        let (versioned, versioned_no_timeout) =
            self.filter_pull_responses(crds, timeouts, response, now, &mut stats);
        self.process_pull_responses(crds, from, versioned, versioned_no_timeout, now, &mut stats);
        (stats.failed, stats.timeout_count, stats.success)
    }
}
#[cfg(test)]
mod test {
