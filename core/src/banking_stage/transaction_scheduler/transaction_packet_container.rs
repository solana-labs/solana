...
/// Container for ordering transactions by priority and keeping track of the
/// sanitized transaction, packet, and max age slot for each ID.
...
impl TransactionPacketContainer {
    /// Initialize a new container with the given capacity.
    ...
    /// Get an iterator over the top `n` ids in the queue.
    /// This does not clear the remaining ids from the queue.
