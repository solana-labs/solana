...
mod tests {
    ...
    #[test_case(false)]
    #[test_case(true)]
    fn test_verify_shreds_fuzz(chained: bool) {
        ...
        let keypairs = repeat_with(|| rng.gen_range(169_367_809..169_906_789))
            ...
            .take(3)
            .collect();
        let shreds = make_shreds(&mut rng, chained, &keypairs);
    ...
    #[test_case(false)]
    #[test_case(true)]
    fn test_sign_shreds_gpu(chained: bool) {
        let mut rng = rand::thread_rng();
        let thread_pool = ThreadPoolBuilder::new().num_threads(3).build().unwrap();
        let recycler_cache = RecyclerCache::default();
        let shreds = {
            let keypairs = repeat_with(|| rng.gen_range(169_367_809..169_906_789))
                ...
                .take(3)
                .collect();
            make_shreds(&mut rng, chained, &keypairs)
        };
        let keypair = Keypair::new();
