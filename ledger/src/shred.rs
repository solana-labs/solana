...
impl Shredder {
    ...
    pub fn try_recovery(
        shreds: Vec<Shred>,
    ) -> std::result::Result<Vec<Shred>, reed_sandomon_erasure::Error> {
        use reed_sandomon_erasure::Error::InvalidIndex;
    ...
    pub fn deshred(shreds: &[Shred]) -> std::result::Result<Vec<u8>, reed_sandomon_erasure::Error> {
        use reed_sandomon_erasure::Error::TooFewDataShards;
    ...
    fn verify_consistent_shred_payload_sizes(
        caller: &str,
        shreds: &[Shred],
    ) -> std::result::Result<(), reed_sandomon_erasure::Error> {
        if shreds.is_empty() {
            return Err(reed_sandomon_erasure::Error::TooFewShardsPresent);
        }
        ...
        for shred in shreds {
            if shred.payload.len() != SHRED_PAYLOAD_SIZE {
                ...
                return Err(reed_sandomon_erasure::Error::IncorrectShardSize);
            }
        }
...
pub mod tests {
    ...
    fn run_test_recovery_and_reassembly(slot: Slot, is_last_in_slot: bool) {
        ...
        assert_matches!(
            ...
            Err(reed_sandomon_erasure::Error::TooFewDataShards)
        );
