...
fn process_entries_with_callback(
    ...
    for entry in entries {
        ...
        for transaction_chunk in entry.transactions.chunks(500) {
            // else loop on processing the entry
            loop {
                let iteration_order = if randomize {
                    let mut iteration_order: Vec<usize> = (0..transaction_chunk.len()).collect();
                ...
                let (batch, num_accounts) = bank.prepare_batch(&transaction_chunk, iteration_order);
                total_transactions += transaction_chunk.len();
            ...
            if total_accounts > 500 || total_transactions > 500 {
                execute_batches(
                    bank,
                    &batches,
                    entry_callback,
...
pub mod tests {
    ...
    fn test_process_entries_2_txes_collision_and_error() {
        ...
        let batch1 = bank.prepare_batch(txs1, None).0;
        for result in batch1.lock_results() {
            assert!(result.is_ok());
        }
        // txs1 and txs2 have accounts that conflict, so we must drop txs1 first
        drop(batch1);
        let batch2 = bank.prepare_batch(txs2, None).0;
    ...
    fn test_get_first_error() {
        ...
        let batch = bank.prepare_batch(&transactions, Some(iteration_order)).0;
        let (
            TransactionResults {
                fee_collection_results,
                ..
    ...
    fn large_number_of_accounts() {
        ...
        let GenesisConfigInfo {
            ...
            mint_keypair,
            ..
        } = create_genesis_config(total_lamports);
        let num_transactions = 1400;
        let num_keys = 64;
        let bank = Arc::new(Bank::new(&genesis_config));
        let keys: Vec<Vec<_>> = (0..num_transactions)
            .into_par_iter()
            .map(|_| (0..num_keys).into_iter().map(|_| Keypair::new()).collect())
            .collect();
        debug!("funding..");
        let num_funding_keypairs = 8;
        let funding_keypairs: Vec<_> = (0..num_funding_keypairs)
            .into_iter()
            .map(|_| {
                let k = Keypair::new();
                bank.transfer(
                    (total_lamports - 10_000) / num_funding_keypairs,
                    &mint_keypair,
                    &k.pubkey(),
                )
                .unwrap();
        ...
        let transactions: Vec<_> = keys
            .par_iter()
            .enumerate()
            .map(|(i, tx_keys)| {
                for (j, key) in tx_keys.iter().enumerate() {
                    let data_size = if j == 0 { 0 } else { 10_000 };
                    let tx = system_transaction::create_account(
                        &funding_keypairs[i % num_funding_keypairs as usize],
                        key,
                        bank.last_blockhash(),
                        1,
                        data_size,
                    ...
                    loop {
                        match bank.process_transaction(&tx) {
                            Ok(_) => break,
                            Err(TransactionError::AccountInUse) => continue,
                            Err(e) => assert!(false, "error {:?}", e),
                        }
                    }
                }
                total.fetch_add(1, Ordering::Relaxed);
                let tx_pubkeys = tx_keys.iter().map(|k| k.pubkey()).collect();
                ...
                );
                {
                    ...
                    if lock.elapsed().as_secs() > 1 {
                        debug!("now: {}", total.load(Ordering::Relaxed));
                        *lock = Instant::now();
                    }
                }
                let mut tx = Transaction::new_unsigned(message);
                tx.signatures[0] = new_rand_sig();
                tx
            })
            .collect();
        ...
        let entries: Vec<_> = transactions
            .chunks(1100)
            .map(|tx_chunk| next_entry(&bank.last_blockhash(), 1, tx_chunk.to_vec()))
            .collect();
        debug!("running..");
        process_entries(&bank, &entries, false, None, None).unwrap();
    }
}
