...
fn bank_forks_from_snapshot(
    ...
    // In both the SnapshotFrom::Dir and SnapshotFrom::ARCHIVE cases, the unneeded snapshot dirs left by the
    // previous validator processes are purged here upfront before booting and constructing a new bank.  This frees
    // up the file system space, giving room to the new accounts files.
    // The new snapshot dirs generated by the current validator process will be purged separately in background by
    // the account_background_service (ABS).

    // In the case of the memory-mounted account_paths and booting from an archive, purging the snapshot dirs upfront
    // is critical to have enough memory space for the new append-vecs from the archive.
