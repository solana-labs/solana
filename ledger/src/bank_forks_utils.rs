...
fn bank_forks_from_snapshot(
    ...
    // previous validator processes are purged here upfront before contructing the new bank, so their file system
    // space is released, giving room to the new accounts files.
    // The new snapshot dirs generated by the current validator process will be purged separately in background by
    //  account_background_service (ABS).
