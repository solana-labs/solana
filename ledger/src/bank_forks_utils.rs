...
fn bank_forks_from_snapshot(
    ...
    // In both the SnapshotFrom::Dir and SnapshotFrom::ARCHIVE cases, the unneeded snapshot dirs generated by the
    // previous validator processes should be purged to release the space in the file system.  The new snapshot dirs
    // generated by the current validator process will purged by the account_background_service (ABS).  The old ones
    // generated by the previous validator processes are purged here upfront before contructing the new bank.

    // In the case of the memory-mounted account_paths and booting from the archive, purging the snapshot dirs upfront
    // is critical to have enough memory space for the new append-vecs from the archives.

    if snapshot_config.snapshot_from == SnapshotFrom::Dir {
        // Purge all the PRE snapshot dirs, and all the POST ones except the highest one.
