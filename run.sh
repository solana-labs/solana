...
ok=true
for program in safecoin-{faucet,genesis,keygen,validator}; do
  $program -V || ok=false
done
$ok || {
  echo
...
export RUST_LOG=${RUST_LOG:-safecoin=info,safecoin_runtime::message_processor=debug} # if RUST_LOG is unset, default to info
