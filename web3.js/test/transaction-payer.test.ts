...
describe('Transaction Payer', () => {
  ...
  it('transaction-payer', async () => {
    ...
    await mockRpcResponse({
      ...
      value: LAMPORTS_PER_SAND - 1,
      withContext: true,
    });

    // accountPayer should be less than LAMPORTS_PER_SAND as it paid for the transaction
    ...
    expect(balance).to.be.at.most(LAMPORTS_PER_SAND);
