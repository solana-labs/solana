...
* Two different versions of a slot must generate a different set of shreds.
...
* Given a particular version of a slot, the resulting merkle trees of two 
different sets of shreds have "functionally unique" markle roots.
...
tick `T_B`, then make a `Orphan(B, T_B.hash, T_B.num_hashes)` request
