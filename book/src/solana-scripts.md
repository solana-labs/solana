...
## Secure Script Execution of Unordered Transactions
...
composing out of order messages.  Since a single script may execute multiple
programs, any change to any of the programs in the execution path could
potentially change the outcome of the script.
...
## Script Instruction Vector Execution
...
The runtime executes the script in a different environment than regular
programs. The runtime keeps track of what is the next expected instruction that
the script should invoke. Once a script calls `process_instruction`, the runtime
compares the issued instruction to the next uncalled instruction in the
transaction instruction vector. If the instructions do not match, the
transaction aborts. Otherwise the instruction is called, and upon completion the
script is resumed from the last point of execution.
