        ...
        loop {
            ...
            if let Some(poh_entry) = record_mixin_res {
                ...
                let transaction_indexes =
                    if let Some(transaction_index) = working_bank.transaction_index {
                        let transaction_index_next_batch =
                            transaction_index.saturating_add(num_transactions);
                        working_bank.transaction_index = Some(transaction_index_next_batch);
                        self.working_bank = Some(working_bank.clone());
                        (transaction_index..transaction_index_next_batch).collect()
                    } else {
                        vec![]
                    };
