...
impl Page {
    fn as_mut<'t>(&mut self, token: &'t mut Token) -> &'t mut PageInner {
...
impl LockAttempt {
    ...
    fn page_mut(&mut self, token: &'t mut Token) -> &mut PageInner {
...
impl SchedulingStateMachine {
    ...
    pub fn deschedule_task(&mut self, task: &Task) {
        ...
        Self::unlock_after_execution(&mut self.token, &task, &mut self.retryable_task_queue);
    }
    ...
    fn try_lock_for_task(
        ...
        if lock_count < task.lock_attempts_mut(token).len() {
            if matches!(task_source, TaskSource::Runnable) {
                Self::rollback_locking(&mut task.lock_attempts_mut(token)[..lock_count]);
                task.mark_as_contended(token);
                task.index_with_pages(token);
            }
    ...
    fn unlock_after_execution(token: &mut Token, task: &Task, retryable_task_queue: &mut TaskQueue) {
        ...
        for unlock_attempt in task.lock_attempts_mut(token) {
            ...
            let heaviest_uncontended_now = unlock_attempt.page_mut().heaviest_still_blocked_task(unsafe { &mut Token::assume_on_the_scheduler_thread() });
