...
impl LockAttempt {
    ...
    fn page_mut<'t>(&mut self, token: &'t mut Token) -> &'t mut PageInner {
...
impl SchedulingStateMachine {
    ...
    fn try_lock_for_task(
        ...
        if lock_count < task.lock_attempts_mut(token).len() {
            if matches!(task_source, TaskSource::Runnable) {
                Self::rollback_locking(token, &mut task.lock_attempts_mut(token)[..lock_count]);
    ...
    fn unlock_after_execution(token: &mut Token, task: &Task, retryable_task_queue: &mut TaskQueue) {
        ...
        for unlock_attempt in task.lock_attempts_mut(token) {
            if should_remove {
                unlock_attempt.page_mut(token).remove_blocked_task(unique_weight);
            }

            let is_unused_now = Self::unlock(token, unlock_attempt);
            if !is_unused_now {
                continue;
            }
