...
On storage saving, the proposed file format also deprecates 48 bytes account
meta data fields as optional fields that require 1 bit for each None field.
This saving applies to both hot and cold accounts.  Offline experiments
by converting the current accounts db to use the new format shows
a 50% to 60% storage size reduction.
