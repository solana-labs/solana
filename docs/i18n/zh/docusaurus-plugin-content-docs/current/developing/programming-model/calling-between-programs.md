...
Safecoin运行时允许程序通过称为跨程序调用的机制相互调用。  程序之间的调用是通过一个程序调用另一个程序的指令来实现的。  调用程序将暂停，直到被调用的程序完成对指令的处理为止。
...
Safecoin的运行时内置了`invoke()`，它负责通过指令的`program_id`字段将给定指令路由到`token`程序。
...
例如，如果两个用户想要对Safecoin中的游戏结果押注，则他们每个人都必须将其下注的资产转移到将遵守协议的某些中介机构上。 当前，在Safecoin中尚无办法将此中介程序作为程序来实现，因为该中介程序无法将资产转让给获胜者。
