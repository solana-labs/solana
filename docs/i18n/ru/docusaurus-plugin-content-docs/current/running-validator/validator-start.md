...
Демон (`solana-sys-tuner`) включен в бинарный релиз solana. Restart it, _before_ restarting your validator, after each software upgrade to ensure that the latest recommended settings are applied.
...
> Примечание: Вы можете использовать seed фразу [бумажного кошелька](../wallet-guide/paper-wallet.md) для вашего `--identity` и/или `--authorized-voter` ключей. To use these, pass the respective argument as `solana-validator --identity ASK ... --authorized-voter ASK ...` and you will be prompted to enter your seed phrases and optional passphrase.
...
2. Add a 300GB tmpfs parition by adding a new line containing `tmpfs /mnt/solana-accounts tmpfs rw,size=300G,user=sol 0 0` to `/etc/fstab` (assuming your validator is running under the user "sol"). **ВНИМАНИЕ: Если вы неправильно отредактируете /etc/fstab ваша машина может перестать загружаться**
