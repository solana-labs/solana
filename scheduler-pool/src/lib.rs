...
impl SchedulerPool {
    ...
    pub fn new_boxed(poh_recorder: Option<&Arc<RwLock<PohRecorder>>>, log_messages_bytes_limit: Option<usize>, transaction_status_sender: Option<TransactionStatusSender>, replay_vote_sender: Option<ReplayVoteSender>, Arc<PrioritizationFeeCache>) -> Box<dyn InstalledSchedulerPool> {
...
impl Scheduler {
    fn spawn(scheduler_pool: Arc<SchedulerPool>, initial_context: SchedulingContext) -> Self {
        ...
        let s = Self {
            ...
            timings_and_result: Default::default(),
            commit_status,
            checkpoint,
...
impl InstalledScheduler for Scheduler {
    ...
    fn wait_for_termination(&mut self, from_internal: bool, is_restart: bool) -> Option<(ExecuteTimings, Result<()>)> {
        ...
        self.timings_and_result = Some(self.checkpoint.take_restart_value());
