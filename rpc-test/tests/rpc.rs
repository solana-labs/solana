...
fn test_rpc_subscriptions() {
    ...
    rt.spawn(async move {
        ...
        for signature in signature_set_clone {
            ...
            tokio::spawn({
                let cloned_pubsub_client = Arc::clone(&pubsub_client);
                async move {
                    let (mut sig_notifications, sig_unsubscribe) = cloned_pubsub_client
                        .signature_subscribe(
                            &signature,
        ...
        for pubkey in account_set_clone {
            ...
            tokio::spawn({
                let cloned_pubsub_client = Arc::clone(&pubsub_client);
                async move {
                    let (mut account_notifications, account_unsubscribe) = cloned_pubsub_client
                        .account_subscribe(
                            &pubkey,
