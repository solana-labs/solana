...
impl Bank {
    ...
    fn get_current_leader(&self) -> Option<Pubkey> {
        ...
        let slot = leader_scheduler.tick_height_to_slot(live_height + 1);
...
mod tests {
    ...
    fn test_bank_new_with_leader() {
        ...
        let bank = Bank::new(&genesis_block);
        assert_eq!(
            bank.active_fork().get_balance(&genesis_block.mint_id),
            10_000 - dummy_leader_tokens
        );
        assert_eq!(
            bank.active_fork().get_balance(&dummy_leader_id),
    ...
    fn test_process_genesis() {
        ...
        let bank = Bank::new(&genesis_block);
        assert_eq!(bank.active_fork().get_balance(&genesis_block.mint_id), 3);
        assert_eq!(bank.active_fork().get_balance(&dummy_leader_id), 1);
    ...
    fn test_process_ledger_simple() {
        ...
        assert_eq!(bank.active_fork().get_balance(&mint_keypair.pubkey()), 100);
        assert_eq!(
        ...
        assert_eq!(
            bank.active_fork().get_balance(&mint_keypair.pubkey()),
            100 - 3
        );
    ...
    fn test_hash_internal_state() {
        ...
        let mut bank0 = Bank::new(&genesis_block);
        bank0.process_ledger(ledger0).unwrap();
        let mut bank1 = Bank::new(&genesis_block);
