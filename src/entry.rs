...
//! transactions within it. Entries cannot be reordered, and its field `hash_count`
...
/// Each Entry contains three pieces of data. The `hash_count` field is the number
/// of hashes performed since the previous entry.  The `id` field is the result
/// of hashing `id` from the previous entry `hash_count` times.  The `transactions`
/// field points to Transactions that took place shortly before `id` was generated.
///
/// If you divide `hash_count` by the amount of time it takes to generate a new hash, you
/// get a duration estimate since the last Entry. Since processing power increases
/// over time, one should expect the duration `hash_count` represents to decrease proportionally.
...
pub struct Entry {
    ...
    /// The SHA-256 hash `hash_count` after the previous Entry ID.
    pub id: Hash,
...
impl Entry {
    /// Creates the next Entry `hash_count` after `start_hash`.
    pub fn new(
        start_hash: &Hash,
        cur_hashes: u64,
        ...
        let hash_count = cur_hashes + if transactions.is_empty() { 0 } else { 1 };
    ...
    /// Creates the next Tick Entry `hash_count` after `start_hash`.
    pub fn new_mut(
        start_hash: &mut Hash,
        cur_hashes: &mut u64,
        transactions: Vec<Transaction>,
    ...
    /// Creates a Entry from the number of hashes `hash_count` since the previous transaction
    /// and that resulting `id`.
    pub fn new_tick(hash_count: u64, id: &Hash) -> Self {
        Entry {
            hash_count,
            id: *id,
            transactions: vec![],
    ...
    /// Verifies self.id is the result of hashing a `start_hash` `self.hash_count` times.
    ...
    pub fn verify(&self, start_hash: &Hash) -> bool {
        ...
        let ref_hash = next_hash(start_hash, self.hash_count, &self.transactions);
        if self.id != ref_hash {
            warn!(
...
/// Creates the hash `hash_count` after `start_hash`. If the transaction contains
/// a signature, the final hash will be a hash of both the previous ID and
/// the signature.  If hash_count is zero and there's no transaction data,
///  start_hash is returned.
fn next_hash(start_hash: &Hash, hash_count: u64, transactions: &[Transaction]) -> Hash {
...
/// Creates the next Tick or Transaction Entry `hash_count` after `start_hash`.
pub fn next_entry(start_hash: &Hash, hash_count: u64, transactions: Vec<Transaction>) -> Entry {
    assert!(hash_count > 0 || transactions.is_empty());
    Entry {
        hash_count,
        id: next_hash(start_hash, hash_count, &transactions),
        transactions,
        has_more: false,
        pad: [0, 0, 0],
    }
...
mod tests {
    ...
    fn test_next_entry() {
        ...
        assert_eq!(tick.hash_count, 1);
        assert_ne!(tick.id, zero);

        let tick = next_entry(&zero, 0, vec![]);
        assert_eq!(tick.hash_count, 0);
        assert_eq!(tick.id, zero);

        let keypair = KeyPair::new();
        let tx0 = Transaction::new_timestamp(&keypair, Utc::now(), zero);
        let entry0 = next_entry(&zero, 1, vec![tx0.clone()]);
        assert_eq!(entry0.hash_count, 1);
