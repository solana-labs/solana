...
impl BankFork {
    ...
    where
        F: Fn(&VoteState) -> bool,
    {
        let mut all_votes: HashMap<Pubkey, VoteState> = HashMap::new();
        self.deltas.iter().for_each(|c| {
            c.accounts
                .accounts_db
                .read()
                .unwrap()
                ...
                .for_each(|account| {
                    if vote_program::check_id(&account.owner) {
                        if let Ok(vote_state) = VoteState::deserialize(&account.userdata) {
                            if cond(&vote_state) && !all_votes.contains_key(&vote_state.node_id) {
                                all_votes.insert(vote_state.node_id, vote_state);
                            }
                        }
                    }
                });
        });
        all_votes.into_iter().map(|(_, v)| v).collect()
    }
}
#[cfg(test)]
mod test {
