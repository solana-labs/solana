...
mod utils {
    ...
    /// [`Token`]'s `*_mut` getters. Thus, the Rust aliasing rule for `UnsafeCell` can deductively
    /// be proven to be satisfied simply based on the usual borrow checking of the `&mut` reference
    /// of [`Token`] itself via [`::borrow_mut()`](TokenCell::borrow_mut).
    ///
