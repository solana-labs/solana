...
//! count is reached to zero), it immediately marks the address as relocked. Then, it pops out the
//! first element of the FIFO queue of the address and decrement the popped-out task's number of
//! conflicting addresses. After that, if the number reaches to the zero, it means the task is
//! fully finished locking all of its addresses and is routed for re-scheduling.
//!
