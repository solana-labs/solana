        ...
        match self.table.entry(label) {
            Entry::Vacant(entry) => {
                ...
                match &value.value.data {
                    ...
                    CrdsData::RestartHeaviestFork(_, _, _) => {
                        self.restart_heaviest_fork.insert(value.ordinal, entry_index);
                    }
                    _ => (),
                };
            ...
            Entry::Occupied(mut entry) if overrides(&value.value, entry.get()) => {
                ...
                match &value.value.data {
                    ...
                    CrdsData::RestartLastVotedForkSlots(_, _, _, _) => {
                        self.restart_last_voted_fork_slots.remove(&entry.get().ordinal);
                        self.restart_last_voted_fork_slots
                            .insert(value.ordinal, entry_index);
                    }
                    CrdsData::RestartHeaviestFork(_, _, _) => {
    ...
    pub fn remove(&mut self, key: &CrdsValueLabel, now: u64) {
        ...
        if index < size {
            ...
            match value.value.data {
                ...
                CrdsData::RestartLastVotedForkSlots(_, _, _, _) => {
                    self.restart_last_voted_fork_slots.insert(value.ordinal, index);
                }
                CrdsData::RestartHeaviestFork(_, _, _) => {
...
mod tests {
    ...
    fn check_crds_value_indices<R: rand::Rng>(
        ...
        let num_restart_last_voted_fork_slots = crds
            .table
            .values()
            .filter(|v| matches!(v.value.data, CrdsData::RestartLastVotedForkSlots(_, _, _, _)))
            .count();
        ...
        assert_eq!(
            ...
            crds.get_restart_heaviest_fork(&mut Cursor::default()).count()
        );
        ...
        for restart_last_voted_fork_slots in crds.get_restart_last_voted_fork_slots(&mut Cursor::default()) {
    ...
    fn test_crds_value_indices() {
        ...
        assert_eq!(crds.num_purged() + crds.table.len(), 4096);
        assert!(num_inserts > crds.table.len());
        let (num_nodes, num_votes, num_epoch_slots, num_restart_last_voted_fork_slots, num_restart_heaviest_forks) =
        check_crds_value_indices(&mut rng, &crds);
        ...
        assert!(
            num_restart_last_voted_fork_slots > 100,
            "num restart last voted fork slots: {num_restart_last_voted_fork_slots}"
        );
        assert!(
            num_restart_heaviest_forks > 100,
