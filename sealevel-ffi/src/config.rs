...
pub unsafe extern "C" fn sealevel_config_setopt(
    ...
    match key {
        ...
        SEALEVEL_ENABLE_STACK_FRAME_GAPS => (*config).config.enable_stack_frame_gaps = value != 0,
        SEALEVEL_INSTRUCTION_METER_CHECKPOINT_DISTANCE => {
            (*config).config.instruction_meter_checkpoint_distance = value
        }
        SEALEVEL_ENABLE_INSTRUCTION_METER => (*config).config.enable_instruction_meter = value != 0,
        SEALEVEL_ENABLE_INSTRUCTION_TRACING => {
        ...
        SEALEVEL_REJECT_BROKEN_ELFS => (*config).config.reject_broken_elfs = value != 0,
        SEALEVEL_NOOP_INSTRUCTION_RATIO => (*config).config.noop_instruction_rate = value as u32,
        SEALEVEL_SANITIZE_USER_PROVIDED_VALUES => {
        ...
        SEALEVEL_REJECT_CALLX_R10 => (*config).config.reject_callx_r10 = value != 0,
        SEALEVEL_DYNAMIC_STACK_FRAMES => (*config).config.dynamic_stack_frames = value != 0,
        SEALEVEL_ENABLE_SDIV => (*config).config.enable_sdiv = value != 0,
        SEALEVEL_OPTIMIZE_RODATA => (*config).config.optimize_rodata = value != 0,
        SEALEVEL_STATIC_SYSCALLS => (*config).config.static_syscalls = value != 0,
        SEALEVEL_ENABLE_ELF_VADDR => (*config).config.enable_elf_vaddr = value != 0,
    }
}
