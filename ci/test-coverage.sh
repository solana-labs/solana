...
if [[ -z "$CODECOV_TOKEN" ]]; then
  ...
  echo "$BUILDKITE_BUILD_URL" | (curl -Gso /dev/null -w "%{url_effective}" --data-urlencode @- "" || true)
  echo "$BUILDKITE_BUILD_URL" | (curl -Gso /dev/null -w "%{url_effective}" --data-urlencode @- "" || true) | cut -c 3-
  echo "$BUILDKITE_BUILD_URL" | (curl -Gso /dev/null -w "%{url_effective}" --data-urlencode @- "" || true) | cut -c 3- | sed -e 's/%0A//'
