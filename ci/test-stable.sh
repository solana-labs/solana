...
test-stable-perf)
  ...
  _ cargo +"$rust_stable" test  --package solana-local-cluster --test local_cluster --manifest-path=core/Cargo.toml ${V:+--verbose} --features="$ROOT_FEATURES" -- --nocapture --test-threads=1
