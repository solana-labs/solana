...
impl RangeProof {
    ...
    pub fn new(
        ...
        let mut i = 0;
        let mut exp_z = z * z;
        let mut exp_y = *SCALAR_ONE;

        for (amount_i, n_i) in amounts.iter().zip(bit_lengths.iter()) {
            let mut exp_2 = *SCALAR_ONE;

            for j in 0..(*n_i) {
                // `j` is guaranteed to be at most `u64::BITS` (a 6-bit number) and therefore,
                // casting is lossless and right shift can be safely unwrapped
                let a_L_j = Scalar::from(amount_i.checked_shr(j as u32).unwrap() & 1);
                let a_R_j = a_L_j - *SCALAR_ONE;
        ...
        let mut agg_opening = *SCALAR_ZERO;
        let mut exp_z = z;
        ...
        let G_factors: Vec<Scalar> = iter::repeat(*SCALAR_ONE).take(nm).collect();
    ...
    pub fn from_bytes(slice: &[u8]) -> Result<RangeProof, RangeProofVerificationError> {
        ...
        let attempt = Scalar::from_canonical_bytes(util::read32(&slice[4 * 32..]));
        if attempt.is_none().into() {
            return Err(RangeProofVerificationError::Deserialization);
        }
        let t_x = attempt.unwrap();
        let attempt = Scalar::from_canonical_bytes(util::read32(&slice[5 * 32..]));
        if attempt.is_none().into() {
            return Err(RangeProofVerificationError::Deserialization);
        }

        let t_x_blinding = attempt.unwrap();
        let attempt = Scalar::from_canonical_bytes(util::read32(&slice[6 * 32..]));
        if attempt.is_none().into() {
            return Err(RangeProofVerificationError::Deserialization);
        }
        let e_blinding = attempt.unwrap();
