...
use chrono::prelude::*;
use safecoin_config_program::date_instruction::DateConfig;
use safecoin_config_program::get_config_data;
use safecoin_sdk::{
    account::Account,
...
fn verify_date_account(
    ...
    if keyed_account.owner()? != safecoin_config_program::id() {
...
mod tests {
    ...
    use safecoin_config_program::date_instruction;
    ...
    fn test_verify_account_unauthorized() {
        ...
        let account = Account::new_ref(1, 0, &safecoin_config_program::id());
    ...
    fn test_verify_date_account_uninitialized_config() {
        ...
        let date_pubkey = safecoin_sdk::pubkey::new_rand();
        let account = Account::new_ref(1, 0, &safecoin_config_program::id()); // <-- Attack! Zero space.
        let keyed_account = KeyedAccount::new(&date_pubkey, false, &account);
        assert_eq!(
            verify_date_account(&keyed_account, &date_pubkey).unwrap_err(),
    ...
    fn test_verify_date_account_invalid_date_config() {
        ...
        let date_pubkey = safecoin_sdk::pubkey::new_rand();
        let account = Account::new_ref(1, 1, &safecoin_config_program::id()); // Attack! 1 byte, enough to sneak by `get_config_data()`, but not DateConfig deserialize.
        let keyed_account = KeyedAccount::new(&date_pubkey, false, &account);
        assert_eq!(
            verify_date_account(&keyed_account, &date_pubkey).unwrap_err(),
            InstructionError::InvalidAccountData
