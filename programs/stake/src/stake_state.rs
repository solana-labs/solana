...
use {
    ...
    solana_sdk::{
        ...
        clock::{Clock, Epoch},
        feature_set::{stake_merge_with_unmatched_credits_observed, FeatureSet},
        ...
        match self.state()? {
            StakeState::Stake(meta, mut stake) => {
                ...
                let validated_split_info =
                    validate_split_amount(self, split, lamports, &meta, Some(&stake), feature_set)?;
            ...
            StakeState::Initialized(meta) => {
                meta.authorized.check(signers, StakeAuthorize::Staker)?;
                let validated_split_info =
                    validate_split_amount(self, split, lamports, &meta, None, feature_set)?;
                let mut split_meta = meta;
                split_meta.rent_exempt_reserve =
                    validated_split_info.destination_rent_exempt_reserve;
...
mod tests {
    ...
    fn test_split_source_uninitialized() {
        ...
        // splitting an uninitialized account where the destination is the same as the source
        {
            ...
            assert_eq!(
                stake_keyed_account.split(0, &stake_keyed_account, &signers, &feature_set),
                Ok(()),
            );
            assert_eq!(
                stake_keyed_account.split(
    ...
    fn test_split_to_smaller_account_with_rent_exempt_reserve() {
        ...
        let split_lamport_balances = vec![
            ...
            expected_rent_exempt_reserve + minimum_delegation - 1,
            expected_rent_exempt_reserve + minimum_delegation,
        ];
        for initial_balance in split_lamport_balances {
            let split_stake_account = AccountSharedData::new_ref_data_with_space(
                initial_balance,
    ...
    /// minimum delegation (plus rent exempt reserve), the current behavior allows
