...
mod tests {
    ...
    fn test_update_commission() {
        ...
        let serialized = bincode::serialize(&VoteStateVersions::Current(Box::new(
            VoteState::from(vote_state.clone()),
        )))
        .unwrap();
        ...
        let signers: HashSet<Pubkey> = vec![withdrawer_pubkey.clone()].into_iter().collect();
