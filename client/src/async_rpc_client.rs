//! Asynchronously communication with a Solana node over RPC.
//!
...
use {
    crate::{
        ...
        http_sender::AsyncHttpSender,
        mock_sender::{MockAsyncSender, Mocks},
    ...
    bincode::serialize,
    jsonrpc_core::futures::FutureExt,
    log::*,
...
/// An async client of a remote Solana node.
///
/// `RpcClient` communicates with a Solana node over [JSON-RPC], with the
/// [Solana JSON-RPC protocol][jsonprot]. It is the primary async Rust interface
/// for querying and transacting with the network from external programs.
///
...
/// [`send`](RpcClient::send) method for making any [`AsyncRpcRequest`].
///
...
/// the case of `RpcClient`, the actual return value is in the
...
/// # use solana_client::async_rpc_client::RpcClient;
...
/// #   let statuses = rpc_client.get_signature_statuses(&[signature]).await.unwrap().value;
/// # });
/// # Ok::<(), ClientError>(())
/// ```
///
...
pub struct RpcClient {
    sender: Box<dyn AsyncRpcSender + Send + Sync + 'static>,
    config: RpcClientConfig,
    node_version: futures_locks::RwLock<Option<semver::Version>>,
}

impl RpcClient {
    /// Create an `RpcClient` from an [`AsyncRpcSender`] and an [`RpcClientConfig`].
    ///
    /// This is the basic constructor, allowing construction with any type of
    /// `AsyncRpcSender`. Most applications should use one of the other constructors,
    /// such as [`new`] and [`new_mock`], which create an `RpcClient`
    /// encapsulating an [`AsyncHttpSender`] and [`MockAsyncSender`] respectively.
    fn new_sender<T: AsyncRpcSender + Send + Sync + 'static>(
        sender: T,
        config: RpcClientConfig,
    ) -> Self {
        Self {
            sender: Box::new(sender),
            node_version: futures_locks::RwLock::new(None),
            config,
        }
    }
    ...
    pub fn new_with_timeout(url: String, timeout: Duration) -> Self {
        Self::new_sender(
            AsyncHttpSender::new_with_timeout(url, timeout),
    ...
    ///
    /// See the [`MockAsyncSender`] documentation for an explanation of
    ...
    /// Create a mock `RpcClient`.
    ///
    /// See the [`MockAsyncSender`] documentation for an explanation of how it treats
    /// the `url` argument.
    ///
    /// # Examples
    ///
    ...
    pub fn new_mock_with_mocks(url: String, mocks: Mocks) -> Self {
        Self::new_sender(
            MockAsyncSender::new_with_mocks(url, mocks),
            RpcClientConfig::with_commitment(CommitmentConfig::default()),
        )
    }
    ...
    /// # let bob = Keypair::new();
    /// # let lamports = 50;
    /// let latest_blockhash = tokio_test::block_on( async { rpc_client.get_latest_blockhash().await })?;
    /// let tx = system_transaction::transfer(&alice, &bob.pubkey(), lamports, latest_blockhash);
    /// let config = RpcSendTransactionConfig {
    ///     skip_preflight: true,
    ///     .. RpcSendTransactionConfig::default()
    /// };
    /// let signature = tokio_test::block_on( async { rpc_client.send_transaction_with_config(
    ///         &tx,
    ///         config,
    ///     ).await })?;
    /// # Ok::<(), ClientError>(())
    ...
    /// #
    /// #   loop {
    /// #       let confirmed = rpc_client.confirm_transaction(&signature).await.unwrap();
    /// #       if confirmed {
    /// #          break;
    /// #       }
    /// #   }
    ...
    /// #   loop {
    /// #       let commitment_config = CommitmentConfig::processed();
    /// #       let confirmed = rpc_client.confirm_transaction_with_commitment(&signature, commitment_config).await.unwrap();
    /// #       if confirmed.value {
    /// #           break;
    /// #       }
    /// #   }
    ...
    /// # };
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// # let snapshot_slot_info = tokio_test::block_on( async { rpc_client.get_highest_snapshot_slot().await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    pub async fn get_highest_snapshot_slot(&self) -> ClientResult<RpcSnapshotSlotInfo> {
        if self.get_node_version().await? < semver::Version::new(1, 8, 0) {
            #[allow(deprecated)]
    ...
    /// #   let tx = system_transaction::transfer(&alice, &bob.pubkey(), lamports, latest_blockhash);
    /// #   let signature = rpc_client.send_transaction(&tx).await.unwrap();
    /// #   let status = rpc_client.get_signature_status(&signature).await.unwrap();
    /// # });
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// # tokio_test::block_on( async {
    /// #   let latest_blockhash = rpc_client.get_latest_blockhash().await.unwrap();
    /// #   let statuses = rpc_client.get_signature_statuses_with_history(&[signature]).await.unwrap().value;
    ...
    /// # };
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// let slot = tokio_test::block_on( async { rpc_client.get_slot().await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// # let commitment_config = CommitmentConfig::processed();
    /// # tokio_test::block_on( async {
    /// #   let slot = rpc_client.get_slot_with_commitment(commitment_config).await.unwrap();
    /// # });
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// # };
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// let block_height = tokio_test::block_on( async { rpc_client.get_block_height().await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// # let limit = 3;
    /// # tokio_test::block_on( async {
    /// #   let leaders = rpc_client.get_slot_leaders(start_slot, limit).await.unwrap();
    /// # });
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// # tokio_test::block_on( async {
    /// #   let production = rpc_client.get_block_production().await.unwrap();
    /// # });
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// # let start_slot = 1;
    /// # let limit = 3;
    /// # let leader = tokio_test::block_on( async {
    /// #   rpc_client.get_slot_leaders(start_slot, limit).await
    /// # })?;
    /// # let leader = leader[0];
    ...
    /// # };
    /// # let production = tokio_test::block_on( async { rpc_client.get_block_production_with_config(
    /// #     config
    /// # ).await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// let vote_accounts = tokio_test::block_on( async { rpc_client.get_vote_accounts().await })?;
    ...
    /// # };
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// # let supply = tokio_test::block_on( async { rpc_client.supply().await.unwrap() });
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// # let commitment_config = CommitmentConfig::processed();
    /// # let supply = tokio_test::block_on( async { rpc_client.supply_with_commitment(
    /// #       commitment_config,
    /// #   ).await });
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// };
    /// let accounts = tokio_test::block_on( async { rpc_client.get_largest_accounts_with_config(
    ///     config,
    /// ).await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// # };
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// let accounts = tokio_test::block_on( async { rpc_client.get_vote_accounts().await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// # let commitment_config = CommitmentConfig::processed();
    /// # let accounts = tokio_test::block_on( async { rpc_client.get_vote_accounts_with_commitment(
    /// #     commitment_config,
    /// # ).await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// };
    /// let accounts = tokio_test::block_on( async { rpc_client.get_vote_accounts_with_config(
    ///         config,
    ///     ).await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// # };
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// let cluster_nodes = tokio_test::block_on( async { rpc_client.get_cluster_nodes().await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// # let slot = tokio_test::block_on( async { rpc_client.get_slot().await })?;
    /// # let block = tokio_test::block_on( async { rpc_client.get_block(slot).await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// let block = tokio_test::block_on( async { rpc_client.get_block_with_encoding(
    ///     slot,
    ///     encoding,
    /// ).await })?;
    /// # Ok::<(), ClientError>(())
    ...
    ///     commitment: None,
    /// };
    /// let block = tokio_test::block_on( async { rpc_client.get_block_with_config(
    ///     slot,
    ///     config,
    /// ).await })?;
    /// # Ok::<(), ClientError>(())
    ...
    /// let start_slot = 0;
    /// let end_slot = 9;
    /// let blocks = tokio_test::block_on( async { rpc_client.get_blocks(start_slot, Some(end_slot)).await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// let blocks = tokio_test::block_on( async { rpc_client.get_blocks_with_commitment(
    ///     start_slot,
    ///     Some(end_slot),
    ///     commitment_config,
    /// ).await })?;
    ...
    /// let start_slot = 0;
    /// let limit = 10;
    /// let blocks = tokio_test::block_on( async { rpc_client.get_blocks_with_limit(start_slot, limit).await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// let limit = 10;
    /// let commitment_config = CommitmentConfig::confirmed();
    /// let blocks = tokio_test::block_on( async { rpc_client.get_blocks_with_limit_and_commitment(
    ///     start_slot,
    ///     limit,
    ///     commitment_config,
    /// ).await })?;
    ...
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// # let alice = Keypair::new();
    /// let signatures = tokio_test::block_on( async { rpc_client.get_signatures_for_address(
    ///     &alice.pubkey(),
    /// ).await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// #     client_error::ClientError,
    /// #     async_rpc_client::RpcClient,
    /// #     async_rpc_client::GetConfirmedSignaturesForAddress2Config,
    /// # };
    /// # use solana_sdk::{
    /// #     signature::Signer,
    /// #     signer::keypair::Keypair,
    ...
    /// let slot = tokio_test::block_on( async { rpc_client.get_slot().await })?;
    /// let block_time = tokio_test::block_on( async { rpc_client.get_block_time(slot).await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    pub async fn get_block_time(&self, slot: Slot) -> ClientResult<UnixTimestamp> {
        let request = RpcRequest::GetBlockTime;
        let response = self.sender.send(request, json!([slot])).await;
    ...
    /// # };
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// let epoch_info = tokio_test::block_on( async { rpc_client.get_epoch_info().await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// let commitment_config = CommitmentConfig::confirmed();
    /// let epoch_info = tokio_test::block_on( async { rpc_client.get_epoch_info_with_commitment(
    ///     commitment_config,
    /// ).await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    ///     commitment: Some(CommitmentConfig::processed()),
    /// };
    /// let leader_schedule = tokio_test::block_on( async { rpc_client.get_leader_schedule_with_config(
    ///     Some(slot),
    ///     config,
    /// ).await })?;
    /// # Ok::<(), ClientError>(())
    ...
    /// # };
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// let epoch_schedule = tokio_test::block_on( async { rpc_client.get_epoch_schedule().await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// # };
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// let identity = tokio_test::block_on( async { rpc_client.get_identity().await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// # };
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// let inflation_governor = tokio_test::block_on( async { rpc_client.get_inflation_governor().await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// # };
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// let inflation_rate = tokio_test::block_on( async { rpc_client.get_inflation_rate().await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// # let inflation_reward = tokio_test::block_on( async { rpc_client.get_inflation_reward(
    /// #     &addresses,
    /// #     Some(epoch),
    /// # ).await })?;
    /// # Ok::<(), ClientError>(())
    ...
    /// let version = tokio_test::block_on( async { rpc_client.get_version().await })?;
    ...
    /// # };
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// let slot = tokio_test::block_on( async { rpc_client.minimum_ledger_slot().await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// ```
    /// # use solana_client::{
    /// #     async_rpc_client::{self, RpcClient},
    /// #     client_error::ClientError,
    /// # };
    /// # use solana_sdk::{
    /// #     signature::Signer,
    ...
    /// # };
    /// # use std::str::FromStr;
    /// # let mocks = async_rpc_client::create_rpc_client_mocks();
    ...
    /// # let account = tokio_test::block_on( async { rpc_client.get_account(&alice_pubkey).await.unwrap() });
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// # let alice_pubkey = Pubkey::from_str("BgvYtJEfmZYdVKiptmMjxGzv8iQoo4MWjsP3QsTkhhxa").unwrap();
    /// # let commitment_config = CommitmentConfig::processed();
    /// # let account = tokio_test::block_on( async {rpc_client.get_account_with_commitment(
    /// #       &alice_pubkey,
    /// #       commitment_config,
    /// #   ).await
    /// # })?;
    ...
    /// # };
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// let slot = tokio_test::block_on( async { rpc_client.get_max_retransmit_slot().await })?;
    /// # Ok::<(), ClientError>(())
    ...
    /// # };
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// let slot = tokio_test::block_on( async { rpc_client.get_max_shred_insert_slot().await })?;
    /// # Ok::<(), ClientError>(())
    ...
    /// let accounts = tokio_test::block_on( async { rpc_client.get_multiple_accounts(&pubkeys).await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// let pubkeys = vec![alice.pubkey(), bob.pubkey()];
    /// let commitment_config = CommitmentConfig::processed();
    /// let accounts = tokio_test::block_on( async { rpc_client.get_multiple_accounts_with_commitment(
    ///     &pubkeys,
    ///     commitment_config,
    /// ).await })?;
    /// # Ok::<(), ClientError>(())
    ...
    /// };
    /// let accounts = tokio_test::block_on( async { rpc_client.get_multiple_accounts_with_config(
    ///     &pubkeys,
    ///     config,
    /// ).await })?;
    /// # Ok::<(), ClientError>(())
    ...
    /// # let rpc_client = RpcClient::new_mock_with_mocks("succeeds".to_string(), mocks);
    /// let alice_pubkey = Pubkey::from_str("BgvYtJEfmZYdVKiptmMjxGzv8iQoo4MWjsP3QsTkhhxa").unwrap();
    /// let account_data = tokio_test::block_on( async { rpc_client.get_account_data(&alice_pubkey).await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// let data_len = 300;
    /// let balance = tokio_test::block_on( async { rpc_client.get_minimum_balance_for_rent_exemption(data_len).await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// # let alice = Keypair::new();
    /// let balance = tokio_test::block_on( async { rpc_client.get_balance(&alice.pubkey()).await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// # let alice = Keypair::new();
    /// let commitment_config = CommitmentConfig::processed();
    /// let balance = tokio_test::block_on( async { rpc_client.get_balance_with_commitment(
    ///     &alice.pubkey(),
    ///     commitment_config,
    /// ).await })?;
    /// # Ok::<(), ClientError>(())
    ...
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// # let alice = Keypair::new();
    /// let accounts = tokio_test::block_on( async { rpc_client.get_program_accounts(&alice.pubkey()).await })?;
    /// # Ok::<(), ClientError>(())
    /// ```
    ...
    /// };
    /// let accounts = tokio_test::block_on( async { rpc_client.get_program_accounts_with_config(
    ///         &alice.pubkey(),
    ///         config,
    ///     ).await })?;;
    /// # Ok::<(), ClientError>(())
    ...
    async fn poll_balance_with_timeout_and_commitment(
        &self,
        pubkey: &Pubkey,
        ...
        loop {
                ...
                .await
            {
                ...
                Err(e) => {
                    tokio::time::sleep(*polling_frequency).await;
    ...
    pub async fn is_blockhash_valid(
        ...
        let result = if self.get_node_version().await? < semver::Version::new(1, 8, 0) {
    ...
    pub async fn get_fee_for_message(&self, message: &Message) -> ClientResult<u64> {
        if self.get_node_version().await? < semver::Version::new(1, 8, 0) {
            let Fees { fee_calculator, .. } = self.get_fees().await?;
            Ok(fee_calculator
...
mod tests {
    ...
    async fn test_get_recent_blockhash() {
        ...
        #[allow(deprecated)]
        let result = rpc_client.get_recent_blockhash().await;
    ...
    #[tokio::test]
    async fn get_block_production_with_config_no_error() -> ClientResult<()> {
        let rpc_client = RpcClient::new_mock("succeeds".to_string());
    ...
    async fn test_get_latest_blockhash() {
        ...
        #[allow(deprecated)]
        let is_err = rpc_client.get_latest_blockhash().await.is_err();
        assert!(is_err);
    }
}
