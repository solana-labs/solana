...
use {
    crate::{
        quic_client::QuicTpuConnection, rpc_client::RpcClient,
...
pub struct ThinClient {
    ...
    quic_tpu_connections: Vec<QuicTpuConnection>,
...
impl ThinClient {
    ...
    pub fn new(
        rpc_addr: SocketAddr,
        quic_tpu_addr: SocketAddr,
        transactions_socket: UdpSocket,
        quic: bool,
    ) -> Self {
        let quic_tpu_addr = if quic {
            SocketAddr::new(quic_tpu_addr.ip(), quic_tpu_addr.port() + QUIC_PORT_OFFSET)
        } else {
            quic_tpu_addr
        };
        let quic_tpu_connection = QuicTpuConnection::new(transactions_socket, quic_tpu_addr);
        Self::new_from_clients(RpcClient::new_socket(rpc_addr), quic_tpu_connection)
    }
    ...
    pub fn new_socket_with_timeout(
        ...
        let quic_tpu_connection = QuicTpuConnection::new(transactions_socket, quic_tpu_addr);
        Self::new_from_clients(rpc_client, quic_tpu_connection)
    }

    fn new_from_clients(rpc_client: RpcClient, quic_tpu_connection: QuicTpuConnection) -> Self {
        Self {
            rpc_clients: vec![rpc_client],
            quic_tpu_connections: vec![quic_tpu_connection],
    ...
    pub fn new_from_addrs(
        ...
        let quic_tpu_connections: Vec<_> = tpu_addrs
            .into_iter()
            .map(|tpu_addr| {
                QuicTpuConnection::new(
                    transactions_socket.try_clone().unwrap(),
                    if quic {
        ...
        Self {
            rpc_clients,
            quic_tpu_connections,
            optimizer,
        }
    }

    fn quic_tpu_connection(&self) -> &QuicTpuConnection {
        &self.quic_tpu_connections[self.optimizer.best()]
    }
        ...
        for x in 0..tries {
            ...
            while now.elapsed().as_secs() < wait_time as u64 {
                if num_confirmed == 0 {
                    ...
                    self.quic_tpu_connection()
                        .send_transaction_sync(transaction)?;
                }
...
impl Client for ThinClient {
    fn tpu_addr(&self) -> String {
        self.quic_tpu_connection().tpu_addr().to_string()
    }
}
...
impl AsyncClient for ThinClient {
    ...
    fn async_send_batch(&self, transactions: Vec<Transaction>) -> TransportResult<()> {
        self.quic_tpu_connection().send_batch_sync(transactions)
    }
