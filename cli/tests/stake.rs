...
fn test_stake_redelegation_then_deactivation_withdraw_not_permitted() {
    ...
    // wait for one epoch, now stake2_account should be fully activated.
    wait_for_next_epoch_plus_n_slots(&rpc_client, 0);

    // Deactivate stake2_account should succeed.
    config.signers = vec![&default_signer];
...
fn test_stake_redelegation_then_merge_deactivation_not_permitted(deactivating_merge_stake: bool) {
    let mint_keypair = Keypair::new();
    let mint_pubkey = mint_keypair.pubkey();
    let authorized_withdrawer = Keypair::new().pubkey();
    let faucet_addr = run_local_faucet(mint_keypair, None);
    ...
    // Create stake merge account
    let stake_merge_keypair = Keypair::new();
    config.signers = vec![&default_signer, &stake_merge_keypair];
    config.command = CliCommand::CreateStakeAccount {
        stake_account: 1,
        seed: None,
        staker: None,
    ...
    // Delegate stake_merge to `vote_keypair2`
    config.signers = vec![&default_signer];
    config.command = CliCommand::DelegateStake {
        stake_account_pubkey: stake_merge_keypair.pubkey(),
        vote_account_pubkey: vote2_keypair.pubkey(),
        stake_authority: 0,
        force: true,
        sign_only: false,
    ...
    // Deactivate stake_merge_account to test (Activating, Inactive) merge case
    if deactivating_merge_stake {
        config.signers = vec![&default_signer];
    ...
    // merge stake_merge account to stake2_account should NOT clear the the `MustFullyActivateBeforeDeactivationIsPermitted` flag.
    config.signers = vec![&default_signer];

    config.command = CliCommand::MergeStake {
        stake_account_pubkey: stake2_keypair.pubkey(),
        source_stake_account_pubkey: stake_merge_keypair.pubkey(),
        stake_authority: 0,
        sign_only: false,
        dump_transaction_message: true,
        blockhash_query: BlockhashQuery::default(),
    ...
    assert_eq!(
        ...
        RpcStakeActivation {
            state: StakeActivationState::Activating,
            active: 0,
            inactive: 100_000_000_000 - rent_exempt_reserve,
        }
    );
...
#[test]
fn test_stake_redelegation_then_merge_activating_stake_then_deactivation_not_permitted() {
    test_stake_redelegation_then_merge_deactivation_not_permitted(false);
}

#[test]
fn test_stake_redelegation_then_merge_inactive_stake_then_deactivation_not_permitted() {
    test_stake_redelegation_then_merge_deactivation_not_permitted(true);
}
