...
if [[ $cloudProvider = ec2 ]]; then
  ...
  sshPrivateKey="$HOME/.ssh/safecoin-net-id_$prefix"
else
    ...
    if [[ ! -r $sshPrivateKey ]]; then
      ...
      (
        ...
        for i in $(seq 1 60); do
          set -x
          cloud_FetchFile "$nodeName" "$nodeIp" /safecoin-scratch/id_ecdsa "$sshPrivateKey" "$nodeZone" &&
            cloud_FetchFile "$nodeName" "$nodeIp" /safecoin-scratch/id_ecdsa.pub "$sshPrivateKey.pub" "$nodeZone" &&
              break
          set +x
  ...
  recordInstanceIp() {
    ...
    (
      ...
      for i in $(seq 1 60); do
        (
          set -x
          timeout --preserve-status --foreground 20s ssh "${sshOptions[@]}" "$publicIp" "ls -l /safecoin-scratch/.instance-startup-complete"
        )
        ret=$?
        if [[ $ret -eq 0 ]]; then
...
if [[ -f /safecoin-scratch/.instance-startup-complete ]]; then
  echo reboot
  $(
    cd "$here"/scripts/
    if "$enableGpu"; then
  ...
  if [[ -x ~safecoin/safecoin/on-reboot ]]; then
    sudo -u safecoin ~safecoin/safecoin/on-reboot
  fi
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  To block until setup is complete, run:
    $ until [[ -f /safecoin-scratch/.instance-startup-complete ]]; do sleep 1; done

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
$(creationInfo)
EOM

# Place the generated private key at /safecoin-scratch/id_ecdsa so it's retrievable by anybody
# who is able to log into this machine
mkdir -m 0777 /safecoin-scratch
cat > /safecoin-scratch/id_ecdsa <<EOK
$(cat "$sshPrivateKey")
EOK
cat > /safecoin-scratch/id_ecdsa.pub <<EOK
$(cat "$sshPrivateKey.pub")
EOK
chmod 444 /safecoin-scratch/id_ecdsa
...
$(
  ...
  cat \
    ...
    create-safecoin-user.sh \
    safecoin-user-authorized_keys.sh \
    add-testnet-safecoin-user-authorized_keys.sh \
    install-ag.sh \
    install-certbot.sh \
    install-earlyoom.sh \
    install-iftop.sh \
...
cat >/safecoin-scratch/gce-self-destruct.sh <<'EOS'
...
cat >/safecoin-scratch/gce-self-destruct-ps1.sh <<'EOS'
#!/usr/bin/env bash
source "$(dirname "$0")/gce-self-destruct.sh"
gce_self_destruct_ps1
EOS
chmod +x /safecoin-scratch/gce-self-destruct-ps1.sh

# Append MOTD and PS1 replacement to .profile
cat >>~safecoin/.profile <<'EOS'
...
export PS1='\[\e]0;\u@\h: \w\a\]${debian_chroot:+($debian_chroot)}\[\033[01;32m\]\u@\h\[\033[00m\]$(/safecoin-scratch/gce-self-destruct-ps1.sh):\[\033[01;34m\]\w\[\033[00m\]\$ '
EOS
EOSD
    cat <<EOSD
...
tmpfsMountPoint=/mnt/safecoin-accounts
