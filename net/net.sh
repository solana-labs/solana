...
usage() {
  ...
  cat <<EOF
usage: $0 [start|stop] [-f configFile]

Manage a multinode network

 start|stop    - Start or stop the network
 -f configFile - Configuration file created by ./gce.sh
                 (default: $configFile)
...
startLeader() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" -f "$ipAddress" "
      cd solana;
      FORCE=1 ./net/remote/remote_leader.sh
    "
  ) >> "$logFile"
}
...
startValidator() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" -f "$ipAddress" "
      set -x;
      cd solana;
      FORCE=1 ./net/remote/remote_validator.sh $leaderIp
    "
   ) >> "$logFile"
}
...
startClient() {
  ...
  ssh "${sshOptions[@]}" -f "$ipAddress" "
    set -x;
    cd solana;
    FORCE=1 ./net/remote/remote_client.sh $leaderIp
  " >> "$logFile"
}
