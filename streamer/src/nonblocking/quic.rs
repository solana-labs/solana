...
async fn setup_connection(
    ...
    .await
    {
        if let Ok(new_connection) = connecting_result {
            ...
            let table_and_stake = {
                let (some_pubkey, stake, t) =
                    get_connection_stake(&connection, staked_nodes.clone())
                        .map_or((None, 0, 0), |(pubkey, stake, t)| (Some(pubkey), stake, t));

                if stake > 0 {
                    remote_pubkey = some_pubkey;
                    let params = NewConnectionHandlerParams {
                        packet_sender,
                        remote_pubkey,
                        stake,
                        total_stake,
                    ...
                    let params = NewConnectionHandlerParams::new_unstaked(
                        packet_sender.clone(),
                        max_connections_per_peer,
                        stats.clone(),
                    );
