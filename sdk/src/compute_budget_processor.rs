...
use crate::{
    ...
    transaction_meta::{
        ...
        MAX_HEAP_FRAME_BYTES, MAX_LOADED_ACCOUNTS_DATA_SIZE_BYTES,
    },
};
...
pub fn process_compute_budget_instruction<'a>(
    ...
    let enable_request_heap_frame_ix = feature_set.is_active(&enable_request_heap_frame_ix::id())
        || maybe_cluster_type
    ...
    for (i, (program_id, instruction)) in instructions.enumerate() {
        if compute_budget::check_id(program_id) {
            let invalid_instruction_data_error =
                SanitizeError::InstructionError(i as u8, InstructionError::InvalidInstructionData);
            ...
            match try_from_slice_unchecked(&instruction.data) {
                ...
                Ok(ComputeBudgetInstruction::RequestHeapFrame(bytes)) => {
                    ...
                    if sanitize_updated_heap_size(bytes as usize, enable_request_heap_frame_ix) {
