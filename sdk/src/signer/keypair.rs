...
impl Keypair {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, ed25519_dalek::SignatureError> {
        Ok(Self(ed25519_dalek::SigningKey::try_from(bytes)?))
    }
...
pub fn read_keypair<R: Read>(reader: &mut R) -> Result<Keypair, Box<dyn error::Error>> {
    ...
    let dalek_keypair = ed25519_dalek::SigningKey::try_from(bytes.as_slice())
...
pub fn keypair_from_seed(seed: &[u8]) -> Result<Keypair, Box<dyn error::Error>> {
    ...
    let signing_key =
        ed25519_dalek::SigningKey::try_from(&seed[..ed25519_dalek::SECRET_KEY_LENGTH])
