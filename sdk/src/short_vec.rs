...
/// Same as usize, but serialized with 1 to 9 bytes. If the value
/// is above 0x7f, the top bit is set and the remaining value is
/// stored in the follow byte. The following byte follows the same
/// pattern until the 9th byte, which may use all 8 bits to store
/// the remaining 8 bits of the original usize.
struct ShortUsize(usize);
...
mod tests {
    ...
    fn encode_len(len: usize) -> Vec<u8> {
        serialize(&ShortUsize(len)).unwrap()
    }

    fn decode_len(bytes: &[u8]) -> usize {
        let short_len: ShortUsize = deserialize(bytes).unwrap();
        short_len.0
    }

    fn assert_len_encoding(len: usize, bytes: &[u8]) {
        assert_eq!(encode_len(len), bytes, "unexpected usize encoding");
        assert_eq!(decode_len(bytes), len, "unexpected usize decoding");
    }
    ...
    impl<'de, T: Deserialize<'de>> Deserialize<'de> for ShortVec<T> {
        ...
        where
            D: Deserializer<'de>,
        {
            super::deserialize(deserializer).map(ShortVec)
        }
    }
