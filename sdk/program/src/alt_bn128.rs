...
mod target_arch {
    ...
    pub fn alt_bn128_addition(input: &[u8]) -> Result<Vec<u8>, AltBn128Error> {
        ...
        Ok(convert_endianness_64(&result_point_data[..ALT_BN128_ADDITION_OUTPUT_LEN]).to_vec())
    }
    ...
    pub fn alt_bn128_multiplication(input: &[u8]) -> Result<Vec<u8>, AltBn128Error> {
        ...
        .try_into()?;
        let fr = <BigInteger256 as FromBytes>::read(convert_endianness_64(&input[64..96]).as_ref())
            .map_err(|_| AltBn128Error::InvalidInputData)?;
